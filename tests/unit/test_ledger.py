...
class LedgerService:
    ...
    def create_journal_entry(self, debit_account, credit_account, amount, description, transaction_id=None):
        ...
        if not all([debit_account, credit_account, description]):
            return {"status": "failed", "message": "Missing required journal entry details"}
        if debit_account == credit_account:
            return {"status": "failed", "message": "Debit and credit accounts cannot be the same"}
    ...
    def get_journal_entries(self, account_name=None, transaction_id=None):
        ...
        for entry in self.entries:
            match_account = (account_name is None or 
                             entry["debit_account"] == account_name or 
                             entry["credit_account"] == account_name)
            match_transaction = (transaction_id is None or 
    ...
    def reconcile_accounts(self, external_records):
        ...
        internal_summary = {}
        for entry in self.entries:
            internal_summary[entry["transaction_id"]] = internal_summary.get(entry["transaction_id"], 0) + entry["amount"]
        ...
        for ext_record in external_records:
            ...
            if ext_txn_id not in internal_summary:
                discrepancies.append(f"External transaction {ext_txn_id} not found in internal ledger.")
            elif internal_summary[ext_txn_id] != ext_amount:
                discrepancies.append(f"Amount mismatch for transaction {ext_txn_id}: Internal {internal_summary[ext_txn_id]}, External {ext_amount}.")
        
        return {"status": "success", "discrepancies": discrepancies, "count": len(discrepancies)}
...
class TestLedgerService(unittest.TestCase):
    ...
    def test_create_journal_entry_success(self):
        result = self.ledger.create_journal_entry("Cash", "Revenue", 100.00, "Sale of goods", "txn_001")
    ...
    def test_create_journal_entry_missing_details(self):
        result = self.ledger.create_journal_entry("Cash", "Revenue", 100.00, None) # Missing description
    ...
    def test_create_journal_entry_same_accounts(self):
        ...
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["message"], "Debit and credit accounts cannot be the same")
        self.assertEqual(len(self.ledger.entries), 0)
    ...
    def test_get_journal_entries_all(self):
        ...
        self.ledger.create_journal_entry("Expenses", "Cash", 20.00, "Office supplies", "txn_002")
    ...
    def test_reconcile_accounts_missing_internal_transaction(self):
        ...
        self.assertIn("External transaction txn_003 not found in internal ledger.", result["discrepancies"])

    def test_reconcile_accounts_amount_mismatch(self):
        self.ledger.create_journal_entry("Cash", "Revenue", 100.00, "Sale", "txn_001")
        external_records = [
            {"transaction_id": "txn_001", "amount": 90.00} # Mismatch
        ]
        result = self.ledger.reconcile_accounts(external_records)
        self.assertEqual(result["status"], "success")
        self.assertEqual(result["count"], 1)
        self.assertIn("Amount mismatch for transaction txn_001: Internal 100.0, External 90.0.", result["discrepancies"])
