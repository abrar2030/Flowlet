...
class FraudDetectionService:
    ...
    def analyze_transaction_for_fraud(self, transaction_data):
        ...
        try:
            ...
            if prediction_result.get("is_fraudulent"):
                ...
                if score > 0.8:
                    return {"status": "fraud_detected", "score": score, "action": "block_transaction"}
                elif score > 0.5:
                    return {"status": "suspicious", "score": score, "action": "review_transaction"}
                else:
                    return {"status": "clean", "score": score, "action": "allow_transaction"}
            else:
                return {"status": "clean", "score": prediction_result.get("score", 0.0), "action": "allow_transaction"}
    ...
    def get_fraud_report(self, transaction_id):
        ...
        if transaction_id == "fraud_txn_123":
            return {"status": "success", "report": {"transaction_id": transaction_id, "reason": "Unusual spending pattern", "severity": "high"}}
        elif transaction_id == "suspicious_txn_456":
            return {"status": "success", "report": {"transaction_id": transaction_id, "reason": "Multiple small transactions", "severity": "medium"}}
        else:
            return {"status": "not_found", "message": "Fraud report not found for this transaction"}
...
class TestFraudDetectionService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fraud_detection_service = FraudDetectionService(ml_model_api=self.mock_ml_model_api)

    def test_analyze_transaction_fraud_detected(self):
        self.mock_ml_model_api.predict_fraud.return_value = {"is_fraudulent": True, "score": 0.9}
        transaction_data = {"amount": 1000, "location": "NY", "user_id": "user1"}
        result = self.fraud_detection_service.analyze_transaction_for_fraud(transaction_data)
    ...
    def test_analyze_transaction_suspicious(self):
        self.mock_ml_model_api.predict_fraud.return_value = {"is_fraudulent": True, "score": 0.6}
    ...
    def test_analyze_transaction_clean_not_fraudulent(self):
        self.mock_ml_model_api.predict_fraud.return_value = {"is_fraudulent": False, "score": 0.1}
    ...
    def test_analyze_transaction_clean_low_score_fraudulent(self):
        self.mock_ml_model_api.predict_fraud.return_value = {"is_fraudulent": True, "score": 0.4}
    ...
    def test_get_fraud_report_not_found(self):
        ...
        self.assertEqual(result["message"], "Fraud report not found for this transaction")
