...
class PaymentProcessor:
    ...
    def refund_payment(self, transaction_id, amount):
        ...
        return {"status": "success", "refund_id": refund_id, "transaction_id": transaction_id, "amount": amount}
...
class TestPaymentProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.valid_card = {"number": "1234-5678-9012-3456", "expiry": "12/25", "cvv": "123"}
        self.invalid_card = {"number": "invalid", "expiry": "12/25", "cvv": "123"}

    def test_process_payment_success(self):
        result = self.processor.process_payment(100.00, "USD", self.valid_card, "user123")
    ...
    def test_process_payment_invalid_card(self):
        result = self.processor.process_payment(50.00, "EUR", self.invalid_card, "user456")
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["message"], "Invalid card details")

    def test_process_payment_missing_details(self):
        result = self.processor.process_payment(10.00, "GBP", {}, "user789") # Missing card details
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["message"], "Missing required payment details")

        result = self.processor.process_payment(10.00, "GBP", self.valid_card, None) # Missing user_id
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["message"], "Missing required payment details")
    ...
    def test_refund_payment_success(self):
        ...
        payment_result = self.processor.process_payment(100.00, "USD", self.valid_card, "user123")
