    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for tx in transactions[:3]:  # Show first 3 transactions
                print(f"  - {tx.get('transaction_type')}: ${tx.get('amount')} - {tx.get('description')}")
        else:
            print(f"Error: {response.text}")
        return response.status_code == 200
    except Exception as e:
...
def test_transfer(from_wallet_id, to_wallet_id, amount):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"From wallet new balance: ${data.get('from_wallet', {}).get('new_balance')}")
            print(f"To wallet new balance: ${data.get('to_wallet', {}).get('new_balance')}")
        else:
            print(f"Error: {response.text}")
        return response.status_code == 200
    except Exception as e:
