...
class FlowletProductionTester:
    ...
    def test_health_and_info(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test("Health Check", True, f"Status: {health_data.get('status')}")
            else:
    ...
    def test_user_registration(self):
        ...
        response = self.make_request('POST', '/auth/register', registration_data, auth=False)
        ...
        if response and response.status_code == 201:
            ...
            if self.access_token and self.test_user_id:
                ...
                self.log_test("User Registration", False, "Missing tokens or user ID", result)
                return False
        else:
            error_data = response.json() if response else None
            self.log_test("User Registration", False, f"HTTP {response.status_code if response else 'No response'}", error_data)
            return False
    ...
    def test_user_login(self):
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("Token Authentication", False, f"HTTP {response.status_code if response else 'No response'}")
            return False
    ...
    def test_account_creation(self):
        ...
        if response and response.status_code == 201:
            ...
            if account_id:
                ...
                self.log_test("Account Creation", False, "No account ID returned", result)
                return None
        else:
            error_data = response.json() if response else None
            self.log_test("Account Creation", False, f"HTTP {response.status_code if response else 'No response'}", error_data)
            return None
    ...
    def test_balance_operations(self, account_id):
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("Balance Inquiry", False, f"HTTP {response.status_code if response else 'No response'}")
            return False
        ...
        response = self.make_request('POST', f'/accounts/{account_id}/deposit', deposit_data)
        ...
        if response and response.status_code == 200:
            ...
            if transaction.get('id'):
                ...
                self.log_test("Fund Deposit", True, f"Amount: $500.00, New Balance: ${new_balance}")
            else:
                self.log_test("Fund Deposit", False, "No transaction ID returned", result)
        else:
            error_data = response.json() if response else None
            self.log_test("Fund Deposit", False, f"HTTP {response.status_code if response else 'No response'}", error_data)
        ...
        response = self.make_request('POST', f'/accounts/{account_id}/withdraw', withdrawal_data)
        ...
        if response and response.status_code == 200:
            ...
            if transaction.get('id'):
                self.test_transactions.append(transaction['id'])
                self.log_test("Fund Withdrawal", True, f"Amount: $200.00, New Balance: ${new_balance}")
            else:
                self.log_test("Fund Withdrawal", False, "No transaction ID returned", result)
        else:
            error_data = response.json() if response else None
            self.log_test("Fund Withdrawal", False, f"HTTP {response.status_code if response else 'No response'}", error_data)
    ...
    def test_transfers(self):
        ...
        if len(self.test_accounts) < 2:
            ...
            if response and response.status_code == 201:
                ...
                if account_id:
                    ...
                    self.log_test("Transfer Setup", False, "Could not create second account")
                    return False
            else:
                self.log_test("Transfer Setup", False, "Could not create second account")
                return False
        ...
        if response and response.status_code == 200:
            ...
            error_data = response.json() if response else None
            self.log_test("Fund Transfer", False, f"HTTP {response.status_code if response else 'No response'}", error_data)
    ...
    def test_transaction_history(self):
        ...
        if response and response.status_code == 200:
            ...
            self.log_test("Transaction History", True, f"Found {len(transactions)} transactions")
            
            # Test pagination
            if pagination.get('total', 0) > 0:
                self.log_test("Transaction Pagination", True, f"Total: {pagination['total']}")
            
            return True
        else:
            error_data = response.json() if response else None
            self.log_test("Transaction History", False, f"HTTP {response.status_code if response else 'No response'}", error_data)
            return False
    ...
    def test_error_handling(self):
        ...
        if response and response.status_code == 404:
            ...
            self.log_test("Invalid Account Error", False, f"Expected 404, got {response.status_code if response else 'No response'}")
        ...
        if self.test_accounts:
            ...
            response = self.make_request('POST', f'/accounts/{self.test_accounts[0]}/withdraw', withdrawal_data)
            
            if response and response.status_code == 400:
                error_data = response.json()
                if error_data.get('code') == 'INSUFFICIENT_FUNDS':
                    self.log_test("Insufficient Funds Error", True, "Correctly detected insufficient funds")
                else:
                    self.log_test("Insufficient Funds Error", False, f"Wrong error code: {error_data.get('code')}")
            else:
                self.log_test("Insufficient Funds Error", False, f"Expected 400, got {response.status_code if response else 'No response'}")
        ...
        if self.test_accounts:
            ...
            response = self.make_request('POST', f'/accounts/{self.test_accounts[0]}/deposit', invalid_deposit_data)
            
            if response and response.status_code == 400:
                self.log_test("Invalid Amount Error", True, "Correctly rejected invalid amount")
            else:
                self.log_test("Invalid Amount Error", False, f"Expected 400, got {response.status_code if response else 'No response'}")
        ...
        if response and response.status_code == 401:
            self.log_test("Unauthorized Access Error", True, "Correctly rejected invalid token")
        else:
            self.log_test("Unauthorized Access Error", False, f"Expected 401, got {response.status_code if response else 'No response'}")
