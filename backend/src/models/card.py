...
class Card(db.Model, TimestampMixin, UUIDMixin):
    ...
    # Limits and controls
    daily_limit_cents = db.Column(
        db.BigInteger, default=100000
    )  # $1,000 default daily limit
    monthly_limit_cents = db.Column(
        db.BigInteger, default=1000000
    )  # $10,000 default monthly limit
    single_transaction_limit_cents = db.Column(
        db.BigInteger, default=50000
    )  # $500 default single transaction limit
    ...
    def is_expired(self):
        ...
        expiry_date = datetime(
            self.expiry_year, self.expiry_month, 1, tzinfo=timezone.utc
        )
            ...
            data.update(
                {
                    ...
                    "single_transaction_limit": float(
                        Decimal(self.single_transaction_limit_cents) / 100
                    ),
                    "total_spent_today": float(
                        Decimal(self.total_spent_today_cents) / 100
                    ),
                    "total_spent_month": float(
                        Decimal(self.total_spent_month_cents) / 100
                    ),
