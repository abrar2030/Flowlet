...
from sqlalchemy import Column, String, DateTime, Text, Boolean, Integer, Index, ForeignKey, Numeric, BigInteger
...
class Account(Base):
    ...
    __tablename__ = 'accounts' # Changed from 'enhanced_accounts' to 'accounts' for simplicity
    ...
    status = Column(db.Enum(AccountStatus), default=AccountStatus.ACTIVE, nullable=False)
    ...
    balance = Column(Numeric(precision=20, scale=8), default=Decimal('0.00000000'), nullable=False)
    available_balance = Column(Numeric(precision=20, scale=8), default=Decimal('0.00000000'), nullable=False)
    pending_balance = Column(Numeric(precision=20, scale=8), default=Decimal('0.00000000'), nullable=False)
    
    # Limits and restrictions (also using Decimal)
    daily_limit = Column(Numeric(precision=20, scale=2), default=Decimal('5000.00'), nullable=False)
    monthly_limit = Column(Numeric(precision=20, scale=2), default=Decimal('50000.00'), nullable=False)
    yearly_limit = Column(Numeric(precision=20, scale=2), default=Decimal('500000.00'), nullable=False)
    
    # Interest and fees (for savings/credit accounts)
    interest_rate = Column(Numeric(5, 4), default=Decimal('0.0000'))  # Annual interest rate
    overdraft_limit = Column(Numeric(20, 2), default=Decimal('0.00'))        # Overdraft limit
    minimum_balance = Column(Numeric(20, 2), default=Decimal('0.00'))        # Minimum balance requirement
    ...
    user = relationship("User", back_populates="wallets") # Assuming User model has 'wallets' relationship
    ...
    def to_dict(self, include_sensitive=False):
        ...
        data = {
            'id': self.id,
            'account_number': self.account_number[-4:] if not include_sensitive else self.account_number,
