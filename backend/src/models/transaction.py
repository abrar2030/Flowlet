...
from sqlalchemy import Column, String, DateTime, Text, Boolean, Integer, Index, ForeignKey, Numeric, Date
...
class Transaction(Base):
    ...
    __tablename__ = 'transactions' # Changed from 'enhanced_transactions' to 'transactions'
    ...
    card_id = Column(String(36), ForeignKey('cards.id'), nullable=True)  # If card transaction
    ...
    status = Column(db.Enum(TransactionStatus), default=TransactionStatus.PENDING, nullable=False)
    ...
    original_amount = Column(Numeric(precision=20, scale=8))  # Original amount in source currency
    ...
    related_transactions = relationship('Transaction', backref=db.backref('parent_transaction', remote_side=[id]))
