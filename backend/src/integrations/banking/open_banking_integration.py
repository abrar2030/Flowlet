...
class OpenBankingIntegration(BankingIntegrationBase, PSD2ComplianceBase, OpenBankingBase):
    """
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    self.logger.error(f"Open Banking authentication failed: {error_data}")
                    raise AuthenticationError(f"Open Banking authentication failed: {error_data}")
    ...
    async def _ensure_authenticated(self):
        ...
        if not self._authenticated or (self.token_expires_at and datetime.now() >= self.token_expires_at):
    ...
    async def initiate_sca(self, customer_id: str, transaction_data: Dict) -> str:
        ...
        async with self.session.post(
            f"{self.base_url}/v1/sca/initiate",
            headers=headers,
            json=sca_data
        ) as response:
            if response.status == 200:
    ...
    async def get_account_balance(self, consent_id: str, account_id: str) -> Dict[str, float]:
        """
            ...
            if response.status == 200:
                ...
                for txn_data in result['transactions']['booked']:
                    ...
                    transaction = Transaction(
                        ...
                        description=txn_data.get('remittanceInformationUnstructured', ''),
                        timestamp=datetime.fromisoformat(txn_data['bookingDate']),
                        counterparty_name=txn_data.get('creditorName') or txn_data.get('debtorName'),
                        counterparty_account=txn_data.get('creditorAccount', {}).get('iban') or 
                                           txn_data.get('debtorAccount', {}).get('iban'),
                        reference_id=txn_data.get('endToEndId'),
                        metadata={
                            'bankTransactionCode': txn_data.get('bankTransactionCode'),
                            'proprietaryBankTransactionCode': txn_data.get('proprietaryBankTransactionCode')
                        }
                    )
                ...
                error_data = await response.json()
                raise BankingIntegrationError(f"Failed to get transactions: {error_data}")
    ...
    async def get_account_information(
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                error_data = await response.json()
                raise BankingIntegrationError(f"Failed to get account information: {error_data}")
    ...
    async def initiate_payment_pis(
        ...
        payment_data = {
            ...
            'debtorAccount': {
                'iban': payment_request.from_account
            },
            'creditorAccount': {
                'iban': payment_request.to_account
            },
            'remittanceInformationUnstructured': payment_request.description,
            'endToEndIdentification': payment_request.reference_id or self.generate_reference_id()
        }
        
        if payment_request.scheduled_date:
            payment_data['requestedExecutionDate'] = payment_request.scheduled_date.strftime('%Y-%m-%d')
        ...
        async with self.session.post(
            ...
            if response.status == 201:
                ...
                error_data = await response.json()
                raise BankingIntegrationError(f"Failed to initiate payment: {error_data}")
    ...
    async def initiate_payment(self, payment_request: PaymentRequest) -> str:
        ...
        consent_id = payment_request.metadata.get('consent_id') if payment_request.metadata else None
        if not consent_id:
            raise BankingIntegrationError("Consent ID required for Open Banking payments")
        
        return await self.initiate_payment_pis(consent_id, payment_request)
    
    async def get_payment_status(self, payment_id: str, consent_id: str = None) -> TransactionStatus:
        """
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                return status_mapping.get(result['transactionStatus'], TransactionStatus.PENDING)
            else:
                error_data = await response.json()
                raise BankingIntegrationError(f"Failed to get payment status: {error_data}")
