...
class FDXIntegration(BankingIntegrationBase):
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    raise AuthenticationError(f"FDX authentication failed: {error_data}")
                ...
                for txn_data in result.get('transactions', []):
                    ...
                    transaction = Transaction(
                        ...
                        metadata={
                            ...
                            'merchantCategoryCode': txn_data.get('merchantCategoryCode'),
    ...
    async def initiate_payment(self, payment_request: PaymentRequest) -> str:
        ...
        payment_data = {
            ...
            'referenceNumber': payment_request.reference_id or self.generate_reference_id()
        }
    ...
    async def get_payment_status(self, transaction_id: str) -> TransactionStatus:
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                return status_mapping.get(result.get('status'), TransactionStatus.PENDING)
            else:
                error_data = await response.json()
    ...
    async def get_account_details(self, account_id: str) -> Dict[str, Any]:
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                error_data = await response.json()
                raise BankingIntegrationError(f"Failed to get account details: {error_data}")
    ...
    async def get_customer_info(self, customer_id: str) -> Dict[str, Any]:
        ...
        async with self.session.get(
            ...
            if response.status == 200:
                ...
                error_data = await response.json()
                raise BankingIntegrationError(f"Failed to get customer info: {error_data}")
