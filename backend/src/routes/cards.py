...
from ..models.card import Card, CardType, CardStatus, CardNetwork
from ..models.transaction import Transaction, TransactionType, TransactionStatus, TransactionCategory
from ..security.password_security import hash_password, check_password
from ..security.audit_logger import audit_logger
from ..models.audit_log import AuditEventType, AuditSeverity
from ..utils.validators import InputValidator
from .auth import token_required, admin_required # Assuming decorators are defined here for now
...
def issue_card():
    ...
    try:
        ...
        if not data:
            return jsonify({'error': 'Request body must contain valid JSON', 'code': 'INVALID_JSON'}), 400
        
        required_fields = ['account_id', 'card_type', 'card_network']
        missing_fields = [field for field in required_fields if field not in data]
        if missing_fields:
            return jsonify({'error': f'Missing required fields: {", ".join(missing_fields)}', 'code': 'MISSING_FIELDS'}), 400
        ...
        card = Card(
            ...
            status=CardStatus.ACTIVE if card_type == CardType.VIRTUAL else CardStatus.INACTIVE,
        )
        ...
        if 'per_transaction' in spending_limits:
            card.single_transaction_limit = Decimal(str(spending_limits['per_transaction']))
        ...
        response_data['message'] = f'{card.card_type.value.title()} card issued successfully.'
        
        return jsonify(response_data), 201

    except IntegrityError:
        db.session.rollback()
        return jsonify({'error': 'A card with this token already exists', 'code': 'INTEGRITY_ERROR'}), 409
    except Exception as e:
        db.session.rollback()
        logger.error(f"Issue card error: {str(e)}", exc_info=True)
        return jsonify({'error': 'An unexpected error occurred during card issuance', 'code': 'INTERNAL_ERROR'}), 500
...
def update_card_status(card_id):
    ...
    try:
        ...
        if not data or 'status' not in data:
            return jsonify({'error': 'Missing status field', 'code': 'MISSING_DATA'}), 400
        ...
        try:
            ...
            return jsonify({'error': f'Invalid status. Must be one of: {[s.value for s in CardStatus]}', 'code': 'INVALID_STATUS'}), 400
        ...
        return jsonify({'message': f'Card status updated to {new_status.value}', 'card': card.to_dict()}), 200
        
    except Exception as e:
        db.session.rollback()
        logger.error(f"Update card status error: {str(e)}", exc_info=True)
        return jsonify({'error': 'Failed to update card status', 'code': 'UPDATE_STATUS_ERROR'}), 500
...
def set_card_pin(card_id):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to set card PIN', 'code': 'SET_PIN_ERROR'}), 500
...
def update_card_controls(card_id):
    ...
    try:
        ...
        if 'daily_limit' in data:
            is_valid, message, amount = InputValidator.validate_amount(data['daily_limit'])
            if not is_valid:
                return jsonify({'error': f'Invalid daily limit: {message}', 'code': 'INVALID_LIMIT'}), 400
            card.daily_limit = amount
            
        if 'monthly_limit' in data:
            is_valid, message, amount = InputValidator.validate_amount(data['monthly_limit'])
            if not is_valid:
                return jsonify({'error': f'Invalid monthly limit: {message}', 'code': 'INVALID_LIMIT'}), 400
            card.monthly_limit = amount
            
        if 'single_transaction_limit' in data:
            is_valid, message, amount = InputValidator.validate_amount(data['single_transaction_limit'])
            if not is_valid:
                return jsonify({'error': f'Invalid single transaction limit: {message}', 'code': 'INVALID_LIMIT'}), 400
            card.single_transaction_limit = amount
            
        # Update boolean controls
        for field in ['is_contactless_enabled', 'is_online_enabled', 'is_international_enabled', 'fraud_alerts_enabled', 'velocity_checks_enabled', 'location_verification_enabled']:
        ...
        return jsonify({'message': 'Card controls updated successfully', 'card': card.to_dict(include_sensitive=True)}), 200
        
    except Exception as e:
        db.session.rollback()
        logger.error(f"Update card controls error: {str(e)}", exc_info=True)
        return jsonify({'error': 'Failed to update card controls', 'code': 'UPDATE_CONTROLS_ERROR'}), 500
