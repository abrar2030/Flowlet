...
def process_payment():
    ...
    try:
        ...
        required_fields = ['account_id', 'amount', 'currency', 'payment_method', 'payment_details']
        ...
        if not account or account.user_id != g.current_user.id:
            return jsonify({'error': 'Account not found or access denied', 'code': 'ACCOUNT_ACCESS_DENIED'}), 403
        ...
        audit_logger.log_event(
            ...
            details={'account_id': account_id, 'amount': float(amount), 'status': result.get('status')}
        )
        ...
        return jsonify({'error': 'An unexpected error occurred during payment processing', 'code': 'INTERNAL_ERROR'}), 500
...
def payment_webhook(processor_name):
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': 'Invalid processor name', 'code': 'INVALID_PROCESSOR'}), 404
        ...
        logger.error(f"Payment webhook error for {processor_name}: {str(e)}", exc_info=True)
...
def get_transaction_details(transaction_id):
    ...
    try:
        ...
        if not transaction:
            return jsonify({'error': 'Transaction not found', 'code': 'TRANSACTION_NOT_FOUND'}), 404
        ...
        return jsonify({'error': 'Failed to retrieve transaction details', 'code': 'INTERNAL_ERROR'}), 500
