...
from ..models.ledger import LedgerEntry, AccountType # Assuming LedgerEntry and AccountType are now in models/ledger.py
...
def create_journal_entry():
    ...
    try:
        ...
        if not data or 'entries' not in data:
            return jsonify({'error': 'Missing entries data', 'code': 'MISSING_DATA'}), 400
        ...
        for entry_data in entries_data:
            ...
            if account_name not in CHART_OF_ACCOUNTS:
                return jsonify({'error': f'Invalid account name: {account_name}', 'code': 'INVALID_ACCOUNT'}), 400
            
            # 3. Validate currency
            if not entry_data.get('currency'):
                return jsonify({'error': 'Missing currency for entry', 'code': 'MISSING_CURRENCY'}), 400
        
        if total_debits != total_credits:
            return jsonify({'error': f'Unbalanced entry: Debits ({total_debits}) != Credits ({total_credits})', 'code': 'UNBALANCED_ENTRY'}), 400
        
        if len(entries_data) < 2:
            return jsonify({'error': 'Journal entry must have at least two entries', 'code': 'INSUFFICIENT_ENTRIES'}), 400
        ...
        for entry_data in entries_data:
            ...
            ledger_entry = LedgerEntry(
                ...
                description=entry_data.get('description', data.get('description', 'Manual Journal Entry')),
...
def get_account_balance(account_name):
    ...
    try:
        if account_name not in CHART_OF_ACCOUNTS:
            return jsonify({'error': f'Account {account_name} not found', 'code': 'INVALID_ACCOUNT'}), 404
        
        account_info = CHART_OF_ACCOUNTS[account_name]
        
        # Determine if the account is a debit-normal or credit-normal account
        is_debit_normal = account_info['type'] in [AccountType.ASSET, AccountType.EXPENSE]
        ...
        total_debit = result.total_debit if result and result.total_debit else Decimal('0.00')
        total_credit = result.total_credit if result and result.total_credit else Decimal('0.00')
