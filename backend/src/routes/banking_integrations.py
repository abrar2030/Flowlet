...
from ..integrations.banking import (AuthenticationError,
                                    BankingIntegrationError,
                                    InvalidAccountError, PaymentRequest)
from ..integrations.banking.manager import IntegrationType, banking_manager
...
def register_integration():
    ...
    try:
        ...
        banking_manager.register_integration(
            data["name"], integration_type, data["config"]
        )
...
def authenticate_integrations():
    ...
    try:
        ...
        return jsonify({"success": True, "authentication_results": results}), 200
...
def get_transactions():
    ...
    try:
        ...
        return jsonify({"success": True, "transactions": serialized_transactions}), 200
...
def initiate_payment():
    ...
    try:
        ...
        payment_request = PaymentRequest(
            ...
            scheduled_date=(
                datetime.fromisoformat(data["scheduled_date"].replace("Z", "+00:00"))
...
def get_payment_status():
    ...
    try:
        ...
        serialized_status = {
            integration: status.value for integration, status in status_results.items()
        }

        return jsonify({"success": True, "payment_status": serialized_status}), 200
...
def health_check():
    ...
    try:
        ...
        all_healthy = all(
            status.get("authenticated", False) for status in health_status.values()
        )
...
def handle_auth_error(e):
    return (
        jsonify(
            {"success": False, "error": "Authentication failed", "details": str(e)}
        ),
        401,
    )
...
def handle_invalid_account_error(e):
    return (
        jsonify({"success": False, "error": "Invalid account", "details": str(e)}),
        404,
    )
...
def handle_banking_error(e):
    return (
        jsonify(
            {"success": False, "error": "Banking integration error", "details": str(e)}
        ),
        400,
    )
