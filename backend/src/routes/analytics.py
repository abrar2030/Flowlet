...
from src.models.database import Card, Transaction, User, Wallet, db
...
def get_dashboard_analytics(user_id):
    ...
    try:
        ...
        for wallet in wallets:
            spending = db.session.query(db.func.sum(Transaction.amount)).filter_by(
                wallet_id=wallet.id, transaction_type="debit"
            ).filter(Transaction.created_at >= current_month_start).scalar() or Decimal(
                "0.00"
            )
        ...
        active_cards = (
            Card.query.join(Wallet)
            .filter(Wallet.user_id == user_id, Card.status == "active")
            .count()
        )
...
def get_spending_analytics(user_id):
    ...
    try:
        ...
        for transaction in spending_transactions:
            ...
            if any(
                word in description for word in ["restaurant", "food", "dining", "cafe"]
            ):
                category_spending["food_dining"] += transaction.amount
            elif any(
                word in description for word in ["store", "shop", "retail", "amazon"]
            ):
                category_spending["shopping"] += transaction.amount
            elif any(
                word in description
                for word in ["gas", "fuel", "uber", "taxi", "transport"]
            ):
                category_spending["transportation"] += transaction.amount
            elif any(
                word in description
                for word in ["movie", "game", "entertainment", "netflix"]
            ):
                category_spending["entertainment"] += transaction.amount
            elif any(
                word in description
                for word in ["bill", "utility", "electric", "water", "internet"]
            ):
    ...
    try:
        ...
        for transaction in income_transactions:
            ...
            if any(word in description for word in ["salary", "payroll", "wage"]):
                ...
                word in description for word in ["dividend", "interest", "investment"]
            ):
...
def get_balance_history(user_id):
    ...
    try:
        ...
        while current_date <= datetime.utcnow():
            ...
            simulated_balance = current_total_balance * (
                1 + variation * (days_from_start / 30)
            )
            ...
            balance_history.append(
                {
                    ...
                    "balance": str(
                        max(Decimal("0.00"), Decimal(str(simulated_balance)))
                    ),
                }
            )
