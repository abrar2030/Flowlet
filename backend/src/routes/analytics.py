...
def get_dashboard_analytics():
    ...
    try:
        ...
        monthly_spending = db.session.execute(monthly_spending_stmt).scalar_one_or_none() or Decimal('0.00')
        
        # Get cards count
        total_cards_stmt = select(func.count()).select_from(Card).filter(Card.account_id.in_(account_ids))
        total_cards = db.session.execute(total_cards_stmt).scalar_one()
        
        active_cards_stmt = select(func.count()).select_from(Card).filter(
...
def get_spending_analytics():
    ...
    try:
        ...
        spending_transactions = db.session.execute(spending_transactions_stmt).scalars().all()
        ...
        daily_data = [{'date': date, 'amount': float(amount)} for date, amount in sorted(daily_spending.items())]
        category_data = [{'category': category, 'amount': float(amount)} for category, amount in category_spending.items()]
...
def get_income_analytics():
    ...
    try:
        ...
        income_transactions = db.session.execute(income_transactions_stmt).scalars().all()
        ...
        daily_data = [{'date': date, 'amount': float(amount)} for date, amount in sorted(daily_income.items())]
        source_data = [{'source': source, 'amount': float(amount)} for source, amount in source_income.items()]
...
def get_balance_history():
    ...
    try:
        ...
        total_change = sum(t.amount if t.transaction_type == TransactionType.CREDIT else -t.amount for t in transactions)
        ...
        final_history_list = [{'date': date, 'balance': balance} for date, balance in sorted(final_history.items())]
