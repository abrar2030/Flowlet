...
from src.models.database import db, APIKey, AuditLog
...
def generate_api_key():
    ...
    return 'flw_' + ''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(32))

def hash_api_key(api_key):
    """Hash API key for secure storage"""
    return hashlib.sha256(api_key.encode()).hexdigest()

def log_audit_event(user_id, action, resource_type, resource_id=None, request_data=None, response_status=None):
    """Log audit event"""
    try:
        audit_log = AuditLog(
            user_id=user_id,
...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        key_record = APIKey.query.filter_by(api_key_hash=api_key_hash, is_active=True).first()
        if not key_record:
...
def list_api_keys():
    ...
    try:
        ...
        keys = APIKey.query.order_by(APIKey.created_at.desc())\
...
def update_api_key_permissions(key_id):
    ...
    try:
        ...
        if not key_record.is_active:
            return jsonify({'error': 'Cannot update permissions for inactive API key'}), 400
        
        # Validate permissions
        valid_permissions = ['read', 'write', 'admin', 'wallet', 'payment', 'card', 'kyc', 'ledger']
    ...
    try:
        ...
        if scan_type == 'account':
            ...
            if random.random() < 0.3:  # 30% chance of finding issues
                ...
                scan_results['recommendations'].append('Update password with stronger complexity')
            ...
            if random.random() < 0.1:  # 10% chance
                ...
                scan_results['recommendations'].append('Review transaction for potential fraud')
...
def security_report():
    ...
    try:
        ...
        security_events = AuditLog.query.filter(
            ...
            AuditLog.action.in_([
                'create_api_key', 'revoke_api_key', 'tokenize_data', 
                'detokenize_data', 'security_scan'
            ])
        ).all()
        ...
        security_metrics = {
            ...
            'security_events': {
                ...
                'tokenization_requests': len([e for e in security_events if e.action == 'tokenize_data']),
                'security_scans': len([e for e in security_events if e.action == 'security_scan']),
                'key_operations': len([e for e in security_events if 'api_key' in e.action])
            },
            'compliance_status': {
        ...
        return jsonify({
            ...
            'security_posture': 'strong' if active_api_keys > 0 else 'needs_attention'
        }), 200
