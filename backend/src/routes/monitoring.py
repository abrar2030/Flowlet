...
from src.models.enhanced_database import Transaction, User, Wallet, FraudAlert
...
class TransactionMonitor:
    ...
    def _evaluate_rule(self, rule: Dict[str, Any], transaction_data: Dict[str, Any], user_history: List[Dict]) -> bool:
        ...
        if rule_id == 'high_value_transaction':
            ...
            recent_count = len([t for t in user_history 
                              if (datetime.utcnow() - datetime.fromisoformat(t['created_at'])).total_seconds() < 300])
            ...
            usual_location = self._get_user_usual_location(transaction_data.get('user_id'))
    ...
    def _calculate_ml_risk_score(self, transaction_data: Dict[str, Any], user_history: List[Dict]) -> int:
...
def create_alert_rule():
    ...
    try:
        ...
        if validated_data['severity'] not in ['low', 'medium', 'high', 'critical']:
            return jsonify({'error': 'Invalid severity level', 'code': 'INVALID_SEVERITY'}), 400
...
def resolve_alert(alert_id):
    ...
    try:
        ...
        if validated_data['resolution'] not in ['resolved', 'false_positive', 'escalated']:
...
def get_monitoring_dashboard():
    ...
    try:
        ...
        alert_counts = db.session.query(FraudAlert.severity, db.func.count(FraudAlert.id))\
            .filter(FraudAlert.status == 'open')\
            .group_by(FraudAlert.severity)\
            .all()
        
        # Recent transactions requiring review
        recent_flagged = db.session.query(Transaction)\
