...
def detect_fraud():
    ...
    try:
        ...
        alert = loop.run_until_complete(
            fraud_service.detect_fraud(data, user_history)
        )
        loop.close()
...
def get_model_status():
    ...
    try:
        ...
        if status.get('performance_metrics', {}).get('last_retrain'):
            status['performance_metrics']['last_retrain'] = \
                status['performance_metrics']['last_retrain'].isoformat()
    ...
    try:
        ...
        for level in risk_levels:
            try:
                ...
                return jsonify({
                    'success': False,
                    'error': f'Invalid risk level: {level}'
                }), 400
        
        fraud_service = get_fraud_service()
        alerts = fraud_service.get_recent_alerts(
            hours=hours,
            risk_levels=risk_level_enums if risk_level_enums else None
        )
...
def submit_feedback():
    ...
    try:
        ...
        loop.run_until_complete(
            fraud_service.update_model_feedback(
                transaction_id, is_fraud, feedback_type
            )
        )
        loop.close()
...
def check_retrain_needed():
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        retrain_needed = loop.run_until_complete(
            fraud_service.check_retrain_needed()
        )
        loop.close()
...
def health_check():
    ...
    try:
        ...
        is_healthy = (
            status.get('model_initialized', False) and
            status.get('model_trained', False) and
            status.get('real_time_detector_ready', False)
        )
