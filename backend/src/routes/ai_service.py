...
from src.models.database import FraudAlert, Transaction, User, db
...
def analyze_transaction_fraud():
    ...
    try:
        ...
        recent_transactions = (
            Transaction.query.filter_by(wallet_id=data.get("wallet_id"))
        ...
        if recent_transactions:
            avg_amount = sum(float(t.amount) for t in recent_transactions) / len(
                recent_transactions
            )
...
def get_fraud_alerts():
    ...
    try:
        ...
        alerts = query.order_by(FraudAlert.created_at.desc()).paginate(
...
def analyze_user_behavior():
    ...
    try:
        ...
        if transactions:
            ...
            behavior_patterns["avg_transaction_amount"] = (
                total_amount / debit_count if debit_count > 0 else 0
            )
            ...
            for transaction in transactions:
                ...
                behavior_patterns["preferred_transaction_times"][time_slot] = (
                    behavior_patterns["preferred_transaction_times"].get(time_slot, 0)
                    + 1
                )
            ...
            # Risk indicators
            if (
                len(transactions) > analysis_period * 2
            ):  # More than 2 transactions per day
                behavior_patterns["risk_indicators"].append("high_activity")

            large_transactions = [
                t
                for t in transactions
                if float(t.amount) > behavior_patterns["avg_transaction_amount"] * 5
            ]
            ...
            night_transactions = [
                t
                for t in transactions
                if t.created_at.hour < 6 or t.created_at.hour > 23
            ]
            if (
                len(night_transactions) > len(transactions) * 0.3
            ):  # More than 30% at night
            ...
            jsonify(
                {
                    ...
                    "insights": insights,
                    "risk_score": len(behavior_patterns["risk_indicators"])
                    * 20,  # Simple risk scoring
...
def recommend_financial_products():
    ...
    try:
        ...
        from src.models.database import Card, Wallet
