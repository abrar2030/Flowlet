...
def analyze_transaction_fraud():
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': 'Invalid amount format', 'code': 'INVALID_AMOUNT'}), 400
        ...
        if recent_transactions:
            ...
            avg_amount = total_amount / len(recent_transactions) if len(recent_transactions) > 0 else Decimal('0.00')
    ...
    try:
        ...
        if status:
            try:
                ...
                return jsonify({'error': 'Invalid status filter', 'code': 'INVALID_FILTER'}), 400
        ...
        if risk_level:
            if risk_level == 'high':
                ...
                stmt = stmt.where(and_(FraudAlert.risk_score >= 40, FraudAlert.risk_score < 70))
            elif risk_level == 'low':
                stmt = stmt.where(and_(FraudAlert.risk_score >= 20, FraudAlert.risk_score < 40))
            elif risk_level == 'very_low':
                stmt = stmt.where(FraudAlert.risk_score < 20)
        
        # Execute query with pagination
        offset = (page - 1) * per_page
        paginated_stmt = stmt.order_by(FraudAlert.created_at.desc()).limit(per_page).offset(offset)
        
        alerts = db.session.execute(paginated_stmt).scalars().all()
        
        # Get total count for pagination metadata
        count_stmt = select(func.count()).select_from(FraudAlert).filter(stmt.whereclause)
...
def resolve_fraud_alert(alert_id):
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': 'Invalid resolution status', 'code': 'INVALID_RESOLUTION'}), 400
...
def chatbot_query():
    ...
    try:
        ...
        if 'query' not in data:
            return jsonify({'error': 'Missing required field: query', 'code': 'MISSING_FIELD'}), 400
        ...
        return jsonify({'error': 'AI service failed to process request', 'code': 'AI_PROCESSING_ERROR'}), 500
