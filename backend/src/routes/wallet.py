...
def get_user_accounts():
    ...
    try:
        ...
        return jsonify({'error': 'Failed to retrieve accounts', 'code': 'GET_ACCOUNTS_ERROR'}), 500
...
def get_account_details(account_id):
    ...
    recent_transactions_stmt = select(Transaction).filter_by(account_id=account.id)\
...
def deposit_funds(account_id):
    ...
    try:
        ...
        if not data or 'amount' not in data:
            return jsonify({'error': 'Missing amount field', 'code': 'MISSING_DATA'}), 400
        ...
        if account.status != AccountStatus.ACTIVE:
            return jsonify({'error': 'Account is not active', 'code': 'ACCOUNT_INACTIVE'}), 400
        ...
        return jsonify({'error': 'Failed to process deposit', 'code': 'DEPOSIT_ERROR'}), 500
...
def withdraw_funds(account_id):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to process withdrawal', 'code': 'WITHDRAWAL_ERROR'}), 500
...
def transfer_funds(account_id):
    ...
    try:
        ...
        if not data or 'amount' not in data or 'destination_account_id' not in data:
            return jsonify({'error': 'Missing amount or destination account ID', 'code': 'MISSING_DATA'}), 400
        ...
        if not destination_account:
            return jsonify({'error': 'Destination account not found', 'code': 'DESTINATION_NOT_FOUND'}), 404
        
        # Check account statuses
        if source_account.status != AccountStatus.ACTIVE or destination_account.status != AccountStatus.ACTIVE:
            return jsonify({'error': 'One or both accounts are inactive', 'code': 'ACCOUNT_INACTIVE'}), 400
        
        # Check for sufficient funds
        if source_account.balance < amount:
            return jsonify({'error': 'Insufficient funds in source account', 'code': 'INSUFFICIENT_FUNDS'}), 400
        ...
        if source_account.currency != destination_account.currency:
            ...
            return jsonify({'error': 'Currency mismatch for internal transfer', 'code': 'CURRENCY_MISMATCH'}), 400
        ...
        debit_transaction = Transaction(
            ...
            status=TransactionStatus.COMPLETED,
            description=data.get('description', f'Transfer to {destination_account.account_name}'),
            channel=data.get('channel', 'api'),
        ...
        credit_transaction = Transaction(
            ...
            transaction_category=TransactionCategory.TRANSFER,
            status=TransactionStatus.COMPLETED,
            description=data.get('description', f'Transfer from {source_account.account_name}'),
            channel=data.get('channel', 'api'),
        ...
        return jsonify({'error': 'Failed to process transfer', 'code': 'TRANSFER_ERROR'}), 500
