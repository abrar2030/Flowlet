...
def issue_card():
    ...
    try:
        ...
        card = Card(
            ...
            international_transactions_enabled=controls.get(
                "international_transactions", False
            ),
        ...
        response_data = {
            ...
            "card": {
                ...
                "controls": {
                    ...
                    "blocked_merchant_categories": json.loads(
...
def get_card(card_id):
    ...
    try:
        ...
        recent_transactions = (
            CardTransaction.query.filter_by(card_id=card.id)
                    ...
                    "card": {
                        ...
                        "current_spending": {
                            ...
                            "daily_remaining": float(
                                card.daily_spending_limit - daily_spent
                            ),
                            "monthly_remaining": float(
                                card.monthly_spending_limit - monthly_spent
                            ),
                        },
                        "controls": {
                            "online_transactions_enabled": card.online_transactions_enabled,
                        ...
                        "last_transaction_at": (
                            card.last_transaction_at.isoformat()
        ...
        return (
            jsonify(
                {"error": "Failed to retrieve card details", "code": "GET_CARD_ERROR"}
            ),
            500,
        )
...
def activate_card(card_id):
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to activate card", "code": "ACTIVATION_ERROR"}),
            500,
        )
...
def freeze_card(card_id):
    ...
    try:
        ...
        return jsonify({"error": "Failed to freeze card", "code": "FREEZE_ERROR"}), 500
...
def unfreeze_card(card_id):
    ...
    try:
        ...
        if card.status != CardStatus.BLOCKED:
            return (
                jsonify(
                    {"error": "Card is not currently frozen", "code": "CARD_NOT_FROZEN"}
                ),
                400,
            )
        ...
        audit_logger.log_user_event(
            ...
            details={"card_id": str(card.id), "ip": request.remote_addr},
        )
        ...
        return (
            jsonify({"error": "Failed to unfreeze card", "code": "UNFREEZE_ERROR"}),
            500,
        )
...
def cancel_card(card_id):
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to cancel card", "code": "CANCELLATION_ERROR"}),
            500,
        )
    ...
    try:
        ...
        if "online_transactions" in data:
            ...
            if old_value != card.online_transactions_enabled:
                changes["online_transactions"] = {
                    "old": old_value,
                    "new": card.online_transactions_enabled,
                }
        ...
        if "contactless_payments" in data:
            ...
            if old_value != card.contactless_payments_enabled:
                changes["contactless_payments"] = {
                    "old": old_value,
                    "new": card.contactless_payments_enabled,
                }
        ...
        if "atm_withdrawals" in data:
            ...
            if old_value != card.atm_withdrawals_enabled:
                changes["atm_withdrawals"] = {
                    "old": old_value,
                    "new": card.atm_withdrawals_enabled,
                }

        if "international_transactions" in data:
            old_value = card.international_transactions_enabled
            card.international_transactions_enabled = bool(
                data["international_transactions"]
            )
            if old_value != card.international_transactions_enabled:
                changes["international_transactions"] = {
                    "old": old_value,
                    "new": card.international_transactions_enabled,
                }
...
def update_card_pin(card_id):
    ...
    try:
        ...
        if not data or "new_pin" not in data:
            return (
                jsonify({"error": "New PIN is required", "code": "NEW_PIN_REQUIRED"}),
                400,
            )
        ...
        return (
            jsonify({"error": "Failed to update PIN", "code": "PIN_UPDATE_ERROR"}),
            500,
        )
            ...
            jsonify(
                {
                    ...
                    "summary": {
                        ...
                        "active_cards": len(
                            [c for c in cards if c.status == CardStatus.ACTIVE]
                        ),
                        "blocked_cards": len(
                            [c for c in cards if c.status == CardStatus.BLOCKED]
                        ),
                        "cancelled_cards": len(
                            [c for c in cards if c.status == CardStatus.CANCELLED]
                        ),
                    },
                }
            ),
