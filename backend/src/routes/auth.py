...
def register():
    ...
    try:
        ...
        return (
            jsonify({"error": "Registration failed", "code": "REGISTRATION_ERROR"}),
            500,
        )
...
def login():
    ...
    try:
        ...
        if not user.is_active:
            ...
            return (
                jsonify({"error": "User account is inactive", "code": "USER_INACTIVE"}),
                401,
            )
...
def refresh_token():
    ...
    try:
        ...
        return jsonify({"error": "Token refresh failed", "code": "REFRESH_ERROR"}), 500
...
def get_profile():
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to retrieve profile", "code": "PROFILE_ERROR"}),
            500,
        )
...
def update_profile():
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to update profile", "code": "UPDATE_ERROR"}),
            500,
        )
...
def setup_two_factor():
    ...
    try:
        ...
        totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=user.email, issuer_name="Flowlet Financial"
        )
...
def verify_two_factor():
    ...
    try:
        ...
        if not user.verify_totp(data["code"]):
            return (
                jsonify({"error": "Invalid verification code", "code": "INVALID_CODE"}),
                401,
            )
...
def disable_two_factor():
    ...
    try:
        ...
        # Verify password
        if "password" not in data or not password_manager.verify_password(
...
def verify_token():
    ...
    try:
        ...
        return (
            jsonify(
                {"valid": False, "error": "Token has expired", "code": "TOKEN_EXPIRED"}
            ),
            401,
        )
    except jwt.InvalidTokenError:
        return (
            jsonify(
                {"valid": False, "error": "Token is invalid", "code": "TOKEN_INVALID"}
            ),
            401,
        )
    except Exception as e:
