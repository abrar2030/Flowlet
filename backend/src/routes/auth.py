...
from ..security.password_security import hash_password, check_password, validate_password_strength
...
def register():
    ...
    try:
        ...
        if 'date_of_birth' in data:
            is_valid, message, dob = InputValidator.validate_date(data['date_of_birth'], '%Y-%m-%d')
            if not is_valid:
...
def login():
    ...
    try:
        ...
        if not data or 'email' not in data or 'password' not in data:
            return jsonify({'error': 'Missing email or password', 'code': 'MISSING_CREDENTIALS'}), 400
        ...
        if user.is_locked():
            ...
            return jsonify({'error': 'Account locked due to too many failed attempts', 'code': 'ACCOUNT_LOCKED'}), 401
        ...
        return jsonify({'error': 'An unexpected error occurred during login', 'code': 'INTERNAL_ERROR'}), 500
...
def verify_2fa():
    ...
    try:
        ...
        if not data or 'temp_token' not in data or 'code' not in data:
            return jsonify({'error': 'Missing token or 2FA code', 'code': 'MISSING_DATA'}), 400
        ...
        try:
            ...
            if not user or not user.two_factor_secret:
                return jsonify({'error': 'Invalid user or 2FA not enabled', 'code': '2FA_NOT_ENABLED'}), 400
            ...
            if not totp.verify(code):
                ...
                return jsonify({'error': 'Invalid 2FA code', 'code': 'INVALID_2FA_CODE'}), 401
            ...
            return jsonify({'error': 'Temporary token expired', 'code': 'TOKEN_EXPIRED'}), 401
        except jwt.InvalidTokenError:
            return jsonify({'error': 'Invalid temporary token', 'code': 'TOKEN_INVALID'}), 401
        
    except Exception as e:
        logger.error(f"2FA verification error: {str(e)}", exc_info=True)
        return jsonify({'error': 'An unexpected error occurred during 2FA verification', 'code': 'INTERNAL_ERROR'}), 500
...
def refresh_token():
    ...
    try:
        ...
        if not data or 'refresh_token' not in data:
            return jsonify({'error': 'Missing refresh token', 'code': 'MISSING_TOKEN'}), 400
        
        refresh_token = data['refresh_token']
        
        try:
            new_access_token, new_refresh_token, user_id = token_manager.refresh_tokens(refresh_token)
        ...
        return jsonify({'error': 'An unexpected error occurred during token refresh', 'code': 'INTERNAL_ERROR'}), 500
...
def setup_2fa():
    ...
    try:
        ...
        if user.two_factor_enabled:
            return jsonify({'error': '2FA is already enabled', 'code': '2FA_ALREADY_ENABLED'}), 400
        ...
        return jsonify({'error': 'An unexpected error occurred during 2FA setup', 'code': 'INTERNAL_ERROR'}), 500
...
def enable_2fa():
    ...
    try:
        ...
        if not data or 'secret' not in data or 'code' not in data:
            return jsonify({'error': 'Missing secret or 2FA code', 'code': 'MISSING_DATA'}), 400
        ...
        if not totp.verify(code):
            ...
            return jsonify({'error': 'Invalid 2FA code. Please try again.', 'code': 'INVALID_2FA_CODE'}), 401
        ...
        return jsonify({'error': 'An unexpected error occurred during 2FA enablement', 'code': 'INTERNAL_ERROR'}), 500
...
def disable_2fa():
    ...
    try:
        ...
        if not user.two_factor_enabled:
            return jsonify({'error': '2FA is not enabled', 'code': '2FA_NOT_ENABLED'}), 400
        ...
        return jsonify({'error': 'An unexpected error occurred during 2FA disablement', 'code': 'INTERNAL_ERROR'}), 500
...
def change_password():
    ...
    try:
        ...
        if not data or 'old_password' not in data or 'new_password' not in data:
            return jsonify({'error': 'Missing old or new password', 'code': 'MISSING_DATA'}), 400
        ...
        if not check_password(user.password_hash, old_password):
            ...
            return jsonify({'error': 'Incorrect old password', 'code': 'INCORRECT_PASSWORD'}), 401
        ...
        if check_password(user.password_hash, new_password):
            return jsonify({'error': 'New password cannot be the same as the old password', 'code': 'PASSWORD_REUSE'}), 400
        ...
        return jsonify({'error': 'An unexpected error occurred during password change', 'code': 'INTERNAL_ERROR'}), 500
...
def request_password_reset():
    ...
    try:
        ...
        generic_success = jsonify({'message': 'If an account with that email exists, a password reset link has been sent.'}), 200
        ...
        if current_app.config.get('DEBUG'):
            return jsonify({'message': 'Password reset token generated (DEBUG)', 'reset_token': reset_token}), 200
            
        return generic_success
        
    except Exception as e:
        logger.error(f"Password reset request error: {str(e)}", exc_info=True)
        return jsonify({'error': 'An unexpected error occurred during password reset request', 'code': 'INTERNAL_ERROR'}), 500
...
def confirm_password_reset():
    ...
    try:
        ...
        if not data or 'token' not in data or 'new_password' not in data:
            return jsonify({'error': 'Missing token or new password', 'code': 'MISSING_DATA'}), 400
        ...
        try:
            ...
            return jsonify({'error': 'Password reset token expired', 'code': 'TOKEN_EXPIRED'}), 401
        except jwt.InvalidTokenError:
            return jsonify({'error': 'Invalid password reset token', 'code': 'TOKEN_INVALID'}), 401
        
    except Exception as e:
        db.session.rollback()
        logger.error(f"Password reset confirm error: {str(e)}", exc_info=True)
        return jsonify({'error': 'An unexpected error occurred during password reset confirmation', 'code': 'INTERNAL_ERROR'}), 500
...
def request_email_verification():
    ...
    try:
        ...
        # For development/testing, return the token
        if current_app.config.get('DEBUG'):
            return jsonify({'message': 'Email verification token generated (DEBUG)', 'verification_token': verification_token}), 200
        ...
        return jsonify({'error': 'An unexpected error occurred during email verification request', 'code': 'INTERNAL_ERROR'}), 500
...
def confirm_email_verification():
    ...
    try:
        ...
        if not data or 'token' not in data:
            return jsonify({'error': 'Missing verification token', 'code': 'MISSING_DATA'}), 400
        ...
        try:
            ...
            return jsonify({'error': 'Verification token expired', 'code': 'TOKEN_EXPIRED'}), 401
        except jwt.InvalidTokenError:
            return jsonify({'error': 'Invalid verification token', 'code': 'TOKEN_INVALID'}), 401
        
    except Exception as e:
        db.session.rollback()
        logger.error(f"Email verification confirm error: {str(e)}", exc_info=True)
        return jsonify({'error': 'An unexpected error occurred during email verification confirmation', 'code': 'INTERNAL_ERROR'}), 500
