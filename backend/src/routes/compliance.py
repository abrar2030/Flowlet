...
from src.models.enhanced_database import Transaction, User, KYCRecord, FraudAlert
...
class ComplianceManager:
    ...
    def generate_sar_report(self, user_id: str, suspicious_activity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transactions = db.session.query(Transaction)\
            .filter(Transaction.wallet_id.in_(
    ...
    def screen_against_watchlists(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if screening_results['matches']:
            high_confidence_matches = [m for m in screening_results['matches'] if m['confidence'] >= 80]
    ...
    def _screen_against_source(self, source: str, name: str, user_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def analyze_transaction_patterns(self, user_id: str, days: int = 90) -> Dict[str, Any]:
        ...
        if len(structuring_transactions) >= 3:
            patterns.append({
                ...
                'total_amount': str(sum(t.amount for t in structuring_transactions)),
                'risk_level': 'high'
            })
        ...
        round_amounts = [
            t for t in transactions 
            if t.amount % 1000 == 0 and t.amount >= 5000
        ]
        ...
        for i in range(1, len(transactions)):
            time_diff = (transactions[i].created_at - transactions[i-1].created_at).total_seconds()
        ...
        if len(night_transactions) > len(transactions) * 0.3:  # More than 30% at unusual times
            patterns.append({
                'type': 'unusual_timing',
                'description': 'High percentage of transactions at unusual times',
                'transaction_count': len(night_transactions),
                'percentage': round((len(night_transactions) / len(transactions)) * 100, 2),
                'risk_level': 'low'
            })
        ...
        return {
            ...
            'transaction_summary': {
                ...
                'average_amount': str(sum(t.amount for t in transactions) / len(transactions))
            },
            'patterns': patterns,
    ...
    def _calculate_pattern_risk_level(self, patterns: List[Dict[str, Any]]) -> str:
        ...
        medium_risk_count = len([p for p in patterns if p.get('risk_level') == 'medium'])
...
def generate_sar():
    ...
    try:
        ...
        if 'description' not in activity or not activity['description']:
            raise ValidationError('suspicious_activity.description', 'Field is required')
        ...
        # Generate SAR
        compliance_manager = ComplianceManager()
        sar_report = compliance_manager.generate_sar_report(
            data['user_id'],
            activity
        )
        ...
        AuditLogger.log_event(
            ...
            additional_data={
                ...
                'transaction_count': sar_report['suspicious_activity']['transaction_count']
            }
        )
...
def generate_ctr():
    ...
    try:
        ...
        start_date = InputValidator.validate_date(date_range.get('start_date'), 'start_date')
...
def get_compliance_dashboard():
    ...
    try:
        ...
        pending_kyc = db.session.query(KYCRecord)\
            .filter(KYCRecord.verification_status == 'pending')\
            .count()
        
        # High-risk users
        high_risk_users = db.session.query(User)\
            .filter(User.risk_score >= 70)\
            .count()
        
        # Recent large transactions (>$10k)
        large_transactions = db.session.query(Transaction)\
            .filter(Transaction.amount >= 10000)\
            .filter(Transaction.created_at >= datetime.combine(week_ago, datetime.min.time()))\
            .count()
        
        # Open fraud alerts
        open_alerts = db.session.query(FraudAlert)\
