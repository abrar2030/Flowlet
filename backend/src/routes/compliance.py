...
def generate_sar_report():
    ...
    try:
        ...
        required_fields = ['user_id', 'suspicious_activity_description', 'activity_type']
...
def get_ctr_report():
    ...
    try:
        ...
        transactions_stmt = select(Transaction).filter(
            ...
            Transaction.transaction_type.in_([TransactionType.CREDIT, TransactionType.DEBIT])
...
def watchlist_screening():
    ...
    try:
        ...
        audit_logger.log_event(
            ...
            user_id=g.current_user.id,
            severity=AuditSeverity.MEDIUM if screening_results['overall_status'] != 'clear' else AuditSeverity.LOW,
            resource_type='user',
            resource_id=user_id
        )
