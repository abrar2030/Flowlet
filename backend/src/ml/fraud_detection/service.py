...
class FraudDetectionService:
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            if self._model_exists():
                ...
                model_config = self.config.get('model_config', self._get_default_model_config())
    ...
    async def detect_fraud(
        ...
        try:
            if not self.real_time_detector:
                raise FraudDetectionError("Real-time detector not initialized. Train model first.")
    ...
    async def batch_detect_fraud(
        ...
        try:
            ...
            self.logger.info(f"Batch fraud detection completed for {len(transactions)} transactions")
            return alerts
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'model_trained': self.ensemble_model.is_trained if self.ensemble_model else False,
    ...
    def get_recent_alerts(
        ...
        if risk_levels:
            recent_alerts = [
                ...
                if alert.risk_level in risk_levels
            ]
    ...
    def get_fraud_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_risk_score': np.mean([alert.risk_score for alert in recent_alerts])
        }
    ...
    async def update_model_feedback(
        ...
        transaction_id: str, 
        is_fraud: bool,
        feedback_type: str = "manual_review"
    ):
        """
        ...
        try:
            ...
            elif not is_fraud and alert.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
            ...
            self.logger.info(f"Feedback recorded for transaction {transaction_id}: fraud={is_fraud}")
    ...
    async def check_retrain_needed(self) -> bool:
        ...
        if self.performance_metrics['last_retrain']:
            days_since_retrain = (datetime.now() - self.performance_metrics['last_retrain']).days
        ...
        if hasattr(self, 'feedback_storage') and len(self.feedback_storage) > 100:
            ...
            if false_positive_rate > 0.2:  # 20% threshold
                self.logger.info(f"High false positive rate detected: {false_positive_rate:.2%}")
                return True
