...
def generate_token(user_id):
    ...
    payload = {
        'user_id': user_id,
        'exp': datetime.utcnow() + timedelta(hours=1), # Using a fixed timedelta for now
        'iat': datetime.utcnow()
    }
    return jwt.encode(payload, os.environ.get("JWT_SECRET_KEY", secrets.token_urlsafe(32)), algorithm='HS256')

def verify_token(token):
    """Verify JWT token"""
    try:
        payload = jwt.decode(token, os.environ.get("JWT_SECRET_KEY", secrets.token_urlsafe(32)), algorithms=['HS256'])
    ...
    try:
        ...
        if user.failed_login_attempts >= 5:
            if user.account_locked_until and user.account_locked_until > datetime.now(timezone.utc):
                return jsonify({"success": False, "error": "Account is temporarily locked"}), 423
...
def deposit_money():
    ...
    try:
        ...
        # Verify account belongs to user
        to_account = Account.query.filter_by(id=data["to_account_id"], user_id=user.id).first()
        if not to_account:
            return jsonify({"success": False, "error": "Account not found"}), 404
