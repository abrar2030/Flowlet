...
class AuditLogger:
    ...
    def log_event(
        ...
        try:
            ...
            masked_request_data = (
                AuditLogger._mask_sensitive_data(request_data) if request_data else None
            )
            ...
            audit_entry = AuditLog(
                ...
                request_data=(
                    json.dumps(masked_request_data) if masked_request_data else None
                ),
            ...
            if details:
                audit_entry.request_data = json.dumps(
                    {"details": details, "request_data": masked_request_data}
                )
    ...
    def _mask_sensitive_data(data: Dict) -> Dict:
        ...
        partial_mask_fields = {"email", "phone", "iban", "swift_code"}
        ...
        for key, value in data.items():
            ...
            if key_lower in sensitive_fields:
                ...
                masked_data[key] = [
                    (
                        AuditLogger._mask_sensitive_data(item)
    ...
    def generate_compliance_report(
        start_date: datetime, end_date: datetime, report_type: str = "full"
    ) -> Dict:
        """
        ...
        try:
            ...
            if report_type in ["full", "transactions"]:
                ...
                for event in transaction_events:
                    ...
                    if (
                        action
                        not in report["details"]["transaction_summary"]["by_action"]
                    ):
                        report["details"]["transaction_summary"]["by_action"][
                            action
                        ] = 0
            ...
            if report_type in ["full", "kyc"]:
                ...
                kyc_events = base_query.filter(AuditLog.action.in_(kyc_actions)).all()
            ...
            return {"error": "Failed to generate compliance report", "message": str(e)}
