...
class InputValidator:
    ...
    def validate_routing_number(routing_number: str) -> tuple[bool, str]:
        ...
        def validate_routing_checksum(routing):
            ...
            checksum = sum(
                int(digit) * weight for digit, weight in zip(routing, weights)
            )
    ...
    def validate_address(address_data: Dict[str, str]) -> tuple[bool, str]:
        ...
        if postal_code and not re.match(
            InputValidator.PATTERNS["postal_code"], postal_code.upper()
        ):
    ...
    def validate_transaction_data(transaction_data: Dict[str, Any]) -> tuple[bool, str]:
        ...
        # Validate amount
        is_valid, message, _ = InputValidator.validate_amount(
        ...
        # Validate currency
        is_valid, _ = InputValidator.validate_currency_code(
