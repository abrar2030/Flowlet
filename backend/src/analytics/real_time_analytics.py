...
class MetricWindow:
    ...
    metric_name: str
    aggregation_function: str  # sum, avg, count, min, max
...
class RealTimeAnalytics:
    ...
    def _initialize_default_alert_rules(self):
        ...
        self._alert_rules = {
            'high_transaction_volume': {
                ...
                'threshold': 1000000,  # $1M in 1 minute
            ...
            'high_risk_transaction_spike': {
                ...
                'condition': 'greater_than',
                'threshold': 0.1,  # 10% high risk transactions
            ...
            'fraud_detection_spike': {
                ...
                'condition': 'greater_than',
                'threshold': 0.02,  # 2% fraud detection rate
                'severity': AlertSeverity.CRITICAL,
    ...
    async def _process_transaction_event(self, event: StreamEvent):
        ...
        # Update transaction volume window
        ...
        # Update transaction count window
    ...
    async def _process_system_metric_event(self, event: StreamEvent):
        ...
        if metric_name == 'response_time' and 'response_time_1m' in self._metric_windows:
    ...
    async def _process_fraud_detection_event(self, event: StreamEvent):
        ...
        # Update fraud detection rate window
        ...
        # Immediate alert for confirmed fraud
        if is_fraud:
            ...
            try:
                ...
                for window_name, window in self._metric_windows.items():
                    ...
                    if current_time - window.last_calculation >= window.slide_interval:
                        await self._calculate_window_metric(window_name, window, current_time)
    ...
    async def _calculate_window_metric(self, window_name: str, window: MetricWindow, current_time: datetime):
        ...
        # Calculate metric based on aggregation function
    ...
    async def _check_alert_rule(self, rule_name: str, rule: Dict[str, Any], current_time: datetime):
    ...
    async def _create_alert(self, alert_type: str, severity: AlertSeverity, 
        ...
        # Store active alert
        self._active_alerts[alert_type] = {
            'alert': alert,
    ...
    async def _notify_subscribers(self, event: StreamEvent):
        ...
        if self._dashboard_subscribers:
            self.logger.debug(f"Notifying {len(self._dashboard_subscribers)} subscribers of event {event.event_id}")
    ...
    async def _notify_metric_subscribers(self, metric_name: str, value: float):
        ...
        if subscribers:
            self.logger.debug(f"Notifying {len(subscribers)} subscribers of metric {metric_name}: {value}")
    
    def subscribe_to_dashboard_updates(self, subscriber_id: str):
    ...
    def unsubscribe_from_dashboard_updates(self, subscriber_id: str):
    ...
    def subscribe_to_metric(self, metric_name: str, subscriber_id: str):
    ...
    def unsubscribe_from_metric(self, metric_name: str, subscriber_id: str):
        """Unsubscribe from specific metric updates."""
        
        self._metric_subscribers[metric_name].discard(subscriber_id)
        self.logger.info(f"Subscriber {subscriber_id} removed from metric {metric_name}")
    ...
    def get_active_alerts(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for alert_info in self._active_alerts.values()
        ]
    ...
    def add_custom_metric_window(self, window_name: str, window_config: Dict[str, Any]):
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'metric_subscribers': sum(len(subs) for subs in self._metric_subscribers.values()),
