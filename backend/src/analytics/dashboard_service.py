...
from .data_models import (
    ...
    PerformanceMetrics, AlertConfiguration
)
from .reporting_engine import ReportingEngine, ReportType, ReportParameters
...
class DashboardService:
        ...
        try:
            ...
            for widget_config in dashboard_config.get('widgets', []):
                widget = Widget(
                    ...
                    refresh_interval=RefreshInterval(widget_config.get('refresh_interval', RefreshInterval.NORMAL.value)),
    ...
    async def _get_widget_data(self, widget: Widget) -> Dict[str, Any]:
        ...
        if cache_key in self._widget_cache:
            ...
            if datetime.utcnow() - cache_time < timedelta(seconds=widget.refresh_interval.value):
                return cached_data
    ...
    async def _get_transaction_summary_data(self, widget: Widget) -> Dict[str, Any]:
        ...
        if 'currency' in widget.filters:
            query = query.filter(TransactionAnalytics.currency.in_(widget.filters['currency']))
        if 'transaction_type' in widget.filters:
            query = query.filter(TransactionAnalytics.transaction_type.in_(widget.filters['transaction_type']))
    ...
    async def _get_customer_metrics_data(self, widget: Widget) -> Dict[str, Any]:
        ...
        if widget.type == WidgetType.KPI_CARD:
            if widget.config.get('metric') == 'total_customers':
                ...
                avg_ltv = sum(float(c.predicted_ltv) for c in customers if c.predicted_ltv) / len(customers) if customers else 0
                return {
    ...
    async def _get_risk_metrics_data(self, widget: Widget) -> Dict[str, Any]:
        ...
        high_risk_count = self.db.query(func.count(TransactionAnalytics.id)).filter(
            and_(
                TransactionAnalytics.transaction_date >= start_date,
                TransactionAnalytics.transaction_date <= end_date,
        ...
        # Total transactions
        total_count = self.db.query(func.count(TransactionAnalytics.id)).filter(
            and_(
                TransactionAnalytics.transaction_date >= start_date,
                TransactionAnalytics.transaction_date <= end_date
            )
        ...
        if widget.type == WidgetType.KPI_CARD:
            ...
            if widget.config.get('metric') == 'overall_risk_score':
                avg_risk = self.db.query(func.avg(TransactionAnalytics.risk_score)).filter(
                    and_(
                        TransactionAnalytics.transaction_date >= start_date,
                        TransactionAnalytics.transaction_date <= end_date
                    )
    ...
    async def _get_performance_metrics_data(self, widget: Widget) -> Dict[str, Any]:
        ...
        if widget.type == WidgetType.KPI_CARD:
            if widget.config.get('metric') == 'avg_response_time':
                avg_response = sum(p.response_time_ms for p in performance_data) / len(performance_data) if performance_data else 0
                return {
                ...
                avg_success = sum(float(p.transaction_success_rate) for p in performance_data if p.transaction_success_rate) / len(performance_data) if performance_data else 0
                return {
    ...
    async def _get_alerts_data(self, widget: Widget) -> Dict[str, Any]:
        ...
        # Get active alerts
        active_alerts = self.db.query(AlertConfiguration).filter(
            AlertConfiguration.is_active == True
        ).all()
        
        # Get recent triggered alerts
        recent_alerts = [alert for alert in active_alerts if alert.last_triggered and 
                        alert.last_triggered > datetime.utcnow() - timedelta(hours=24)]
    ...
    def _format_kpi_data(self, transactions: List, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if metric == 'total_transactions':
            ...
            avg = sum(float(t.amount) for t in transactions) / len(transactions) if transactions else 0
            return {
                'value': round(avg, 2),
    ...
    def _format_line_chart_data(self, transactions: List, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for transaction in transactions:
            ...
            time_key = transaction.transaction_date.replace(minute=0, second=0, microsecond=0)
    ...
    def _format_bar_chart_data(self, transactions: List, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _format_pie_chart_data(self, transactions: List, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _format_table_data(self, transactions: List, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        columns = config.get('columns', [
            'transaction_id', 'amount', 'currency', 'transaction_type', 'transaction_date'
        ])
    ...
    async def update_widget(self, dashboard_id: str, widget_id: str, config: Dict[str, Any]) -> bool:
        ...
        for widget in dashboard.widgets:
            if widget.id == widget_id:
                ...
                cache_keys_to_remove = [key for key in self._widget_cache.keys() if key.startswith(widget_id)]
