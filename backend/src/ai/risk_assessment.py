...
class AIRiskAssessor:
    ...
    def assess_user_risk(self, user_id: str, assessment_context: Dict[str, Any] = None) -> RiskAssessment:
        ...
        category_scores[RiskCategory.CREDIT_RISK.value] = self._calculate_category_score(credit_factors)
        
        # Fraud Risk Assessment
        fraud_factors = self._assess_fraud_risk(user_data)
        risk_factors.extend(fraud_factors)
        category_scores[RiskCategory.FRAUD_RISK.value] = self._calculate_category_score(fraud_factors)
        
        # Liquidity Risk Assessment
        liquidity_factors = self._assess_liquidity_risk(user_data)
        risk_factors.extend(liquidity_factors)
        category_scores[RiskCategory.LIQUIDITY_RISK.value] = self._calculate_category_score(liquidity_factors)
        
        # Operational Risk Assessment
        operational_factors = self._assess_operational_risk(user_data)
        risk_factors.extend(operational_factors)
        category_scores[RiskCategory.OPERATIONAL_RISK.value] = self._calculate_category_score(operational_factors)
    ...
    def _assess_credit_risk(self, user_data: Dict[str, Any]) -> List[RiskFactor]:
        ...
        if total_payments > 0:
            ...
            payment_score = max(0, 1 - (late_payment_ratio * 2))  # Penalize late payments
        else:
        ...
        factors.append(RiskFactor(
            ...
            impact="High" if payment_score < 0.5 else "Medium" if payment_score < 0.8 else "Low",
            mitigation_suggestions=[
                "Set up automatic payments to avoid late fees",
                "Monitor payment due dates closely",
                "Maintain sufficient account balances"
            ] if payment_score < 0.7 else []
        ))
        
        # Credit Utilization Analysis
        account_balances = user_data.get('account_balances', {})
        total_balance = sum(Decimal(str(balance)) for balance in account_balances.values())
        
        # Simulate credit limit (in real implementation, get from credit data)
        estimated_credit_limit = Decimal('10000')
        utilization_ratio = float(total_balance / estimated_credit_limit) if estimated_credit_limit > 0 else 0
        
        utilization_score = max(0, 1 - (utilization_ratio * 1.5))  # Penalize high utilization
        
        factors.append(RiskFactor(
            factor_name="credit_utilization",
            category=RiskCategory.CREDIT_RISK,
            weight=self.risk_weights[RiskCategory.CREDIT_RISK]['credit_utilization'],
            score=utilization_score,
            description=f"Credit utilization ratio: {utilization_ratio:.1%}",
            impact="High" if utilization_ratio > 0.7 else "Medium" if utilization_ratio > 0.3 else "Low",
            mitigation_suggestions=[
                "Pay down existing balances",
                "Request credit limit increases",
                "Spread balances across multiple accounts"
            ] if utilization_ratio > 0.5 else []
        ))
    ...
    def _assess_fraud_risk(self, user_data: Dict[str, Any]) -> List[RiskFactor]:
        ...
        factors.append(RiskFactor(
            ...
            impact="High" if device_score < 0.3 else "Medium" if device_score < 0.7 else "Low",
            mitigation_suggestions=[
                "Enable device registration and verification",
                "Monitor for new device logins",
                "Implement device fingerprinting"
            ] if device_score < 0.5 else []
        ))
    ...
    def _assess_liquidity_risk(self, user_data: Dict[str, Any]) -> List[RiskFactor]:
        ...
        factors.append(RiskFactor(
            ...
            weight=self.risk_weights[RiskCategory.LIQUIDITY_RISK]['cash_flow_patterns'],
            score=cash_flow_score,
            description="Analysis of income vs. expenditure patterns",
            impact="High" if cash_flow_score < 0.3 else "Medium" if cash_flow_score < 0.7 else "Low",
            mitigation_suggestions=[
                "Improve income stability",
                "Reduce unnecessary expenses",
                "Build emergency fund reserves"
            ] if cash_flow_score < 0.5 else []
        ))
        ...
        factors.append(RiskFactor(
            ...
            category=RiskCategory.LIQUIDITY_RISK,
            weight=self.risk_weights[RiskCategory.LIQUIDITY_RISK]['account_balances'],
            score=balance_score,
            description="Analysis of account balance levels and trends",
            impact="High" if balance_score < 0.3 else "Medium" if balance_score < 0.7 else "Low",
            mitigation_suggestions=[
                "Maintain higher account balances",
                "Set up overdraft protection",
                "Monitor balance levels regularly"
            ] if balance_score < 0.5 else []
        ))
    ...
    def _assess_operational_risk(self, user_data: Dict[str, Any]) -> List[RiskFactor]:
        ...
        factors.append(RiskFactor(
            ...
            weight=self.risk_weights[RiskCategory.OPERATIONAL_RISK]['security_incidents'],
            score=security_score,
            description=f"Number of security incidents: {len(security_incidents)}",
            impact="High" if len(security_incidents) > 3 else "Medium" if len(security_incidents) > 1 else "Low",
            mitigation_suggestions=[
                "Enhance security awareness training",
                "Implement stronger authentication",
                "Regular security audits"
            ] if len(security_incidents) > 0 else []
        ))
        ...
        factors.append(RiskFactor(
            ...
            category=RiskCategory.OPERATIONAL_RISK,
            weight=self.risk_weights[RiskCategory.OPERATIONAL_RISK]['compliance_violations'],
            score=compliance_score,
            description=f"Number of compliance violations: {len(violations)}",
            impact="High" if len(violations) > 2 else "Medium" if len(violations) > 0 else "Low",
            mitigation_suggestions=[
    ...
    def _analyze_location_patterns(self, location_history: List[Dict]) -> float:
        ...
        for i in range(1, len(sorted_locations)):
            ...
            if prev_loc['country'] != curr_loc['country']:
                time_diff = (datetime.fromisoformat(curr_loc['timestamp']) - 
                           datetime.fromisoformat(prev_loc['timestamp'])).total_seconds() / 3600
    ...
    def _generate_predictions(self, user_data: Dict[str, Any], risk_factors: List[RiskFactor]) -> Dict[str, Any]:
        """Generate AI predictions based on risk assessment"""
        predictions = {}
        
        # Predict default probability
        credit_factors = [f for f in risk_factors if f.category == RiskCategory.CREDIT_RISK]
        ...
        fraud_factors = [f for f in risk_factors if f.category == RiskCategory.FRAUD_RISK]
        ...
        liquidity_factors = [f for f in risk_factors if f.category == RiskCategory.LIQUIDITY_RISK]
    ...
    def _generate_risk_recommendations(self, risk_factors: List[RiskFactor], 
                                     overall_risk_level: RiskLevel) -> List[str]:
        ...
        if overall_risk_level in [RiskLevel.HIGH, RiskLevel.VERY_HIGH, RiskLevel.CRITICAL]:
            ...
            recommendations.append("Consider reducing credit limits or transaction limits")
    ...
    def _generate_monitoring_alerts(self, risk_factors: List[RiskFactor], 
        ...
        for factor in critical_factors:
            alerts.append(f"Critical risk detected in {factor.factor_name}: {factor.description}")
        
        # Prediction-based alerts
        if predictions.get('fraud_likelihood', {}).get('probability', 0) > 0.7:
            alerts.append("High fraud probability detected - implement additional verification")
    ...
    def _calculate_confidence_score(self, user_data: Dict[str, Any], 
    ...
    def _store_assessment(self, assessment: RiskAssessment):
        ...
        if len(self.user_histories[assessment.user_id]) > 50:
            self.user_histories[assessment.user_id] = self.user_histories[assessment.user_id][-50:]
