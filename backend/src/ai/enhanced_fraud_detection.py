...
class ExplainableAIFraudDetector:
    ...
    async def detect_fraud(self, transaction_features: TransactionFeatures) -> FraudDetectionResult:
        """
    ...
    async def _extract_features(self, transaction_features: TransactionFeatures) -> List[float]:
        ...
        location_risk = await self.location_analyzer.calculate_risk(
            transaction_features.location_country,
            transaction_features.location_city
        )
        features.append(location_risk)
        
        # Device risk features
        device_risk = await self.device_profiler.calculate_risk(
            transaction_features.device_fingerprint,
            transaction_features.user_id
        )
    ...
    def _encode_merchant_category(self, category: str) -> int:
        ...
        category_mapping = {
            ...
            'online': 4, 'atm': 5, 'pharmacy': 6, 'hotel': 7,
            'airline': 8, 'entertainment': 9, 'other': 10
        }
        return category_mapping.get(category.lower(), 10)
    
    def _determine_risk_and_action(self, risk_score: float) -> Tuple[RiskLevel, FraudAction]:
    ...
    def _generate_explanation(self, feature_vector: List[float], fraud_probability: float) -> Dict[str, float]:
        """Generate explainable AI explanation for the fraud detection decision"""
        feature_names = [
            'amount', 'merchant_category', 'card_present', 'online_transaction',
            'previous_count_24h', 'previous_amount_24h', 'account_age_days',
            'velocity_score', 'time_since_last', 'location_risk', 'device_risk',
    ...
    def _generate_transaction_id(self, transaction_features: TransactionFeatures) -> str:
    ...
    async def _log_fraud_detection(self, result: FraudDetectionResult, features: TransactionFeatures):
...
class VelocityCalculator:
    ...
    async def calculate_velocity(self, user_id: str, current_amount: float) -> float:
        ...
        try:
            ...
            velocity_score = min(1.0, (current_amount / max(avg_amount, 1)) * (len(recent_amounts) / 10))
