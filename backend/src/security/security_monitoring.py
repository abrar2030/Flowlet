    ...
    def matches(self, event: SecurityEvent) -> bool:
        ...
        for condition_key, condition_value in self.conditions.items():
            if condition_key == 'source' and event.source != condition_value:
                return False
            elif condition_key == 'category' and event.category.value != condition_value:
                return False
...
class SecurityMonitoringService:
    ...
    def _register_default_handlers(self):
        ...
        self.register_event_handler('privilege_escalation', self._handle_privilege_escalation)
        
        # Data access event handlers
        self.register_event_handler('sensitive_data_access', self._handle_sensitive_data_access)
        ...
        self.register_event_handler('configuration_change', self._handle_configuration_change)
    ...
    async def log_security_event(self, event_type: str, category: EventCategory,
                                severity: EventSeverity, source: str, target: str,
                                description: str, details: Dict[str, Any] = None,
                                user_id: str = None, session_id: str = None,
                                ip_address: str = None, user_agent: str = None) -> SecurityEvent:
        """
    ...
    async def _correlate_events(self, event: SecurityEvent):
        ...
        for pattern_name, pattern_config in self._correlation_rules.items():
            if event.event_type in pattern_config['events']:
                await self._check_correlation_pattern(event, pattern_name, pattern_config)
    
    async def _check_correlation_pattern(self, event: SecurityEvent, pattern_name: str,
        ...
        if pattern_name == 'account_takeover_pattern':
            await self._check_account_takeover_pattern(event, recent_events, pattern_config)
        elif pattern_name == 'data_exfiltration_pattern':
            await self._check_data_exfiltration_pattern(event, recent_events, pattern_config)
        elif pattern_name == 'brute_force_pattern':
            await self._check_brute_force_pattern(event, recent_events, pattern_config)
    
    async def _check_account_takeover_pattern(self, event: SecurityEvent,
        ...
        if (len(failed_logins) >= 3 and len(successful_logins) >= 1 and 
            len(password_changes) >= 1 and len(ip_addresses) > 1):
    ...
    async def _check_data_exfiltration_pattern(self, event: SecurityEvent,
        ...
        data_access_events = [e for e in user_events if e.event_type == 'large_data_access']
        download_events = [e for e in user_events if e.event_type == 'file_download']
        transfer_events = [e for e in user_events if e.event_type == 'external_transfer']
        
        if len(data_access_events) >= 1 and (len(download_events) >= 3 or len(transfer_events) >= 1):
            await self._create_correlation_incident(
    ...
    async def _create_correlation_incident(self, title: str, description: str,
                                         severity: EventSeverity, category: EventCategory,
    ...
    async def _update_incident_tracking(self, event: SecurityEvent):
        ...
        if event.severity == EventSeverity.CRITICAL:
            ...
            self._incidents[incident.incident_id] = incident
            self.logger.critical(f"Auto-created incident for critical event: {event.event_id}")
    
    async def _create_or_update_incident(self, rule: AlertRule, events: List[SecurityEvent]):
        ...
        for incident in self._incidents.values():
            if (incident.status in [IncidentStatus.OPEN, IncidentStatus.INVESTIGATING] and
                incident.category == events[0].category):
    ...
    def get_recent_events(self, limit: int = 100, 
                         ...
                         time_range: timedelta = None) -> List[SecurityEvent]:
    ...
    def get_incidents(self, status: IncidentStatus = None,
                     severity: EventSeverity = None,
                     category: EventCategory = None) -> List[SecurityIncident]:
    ...
    def create_alert_rule(self, name: str, description: str, event_type: str,
                         conditions: Dict[str, Any], severity_threshold: EventSeverity,
                         time_window: timedelta, max_events: int) -> str:
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        open_incidents = len([i for i in self._incidents.values() if i.status == IncidentStatus.OPEN])
        critical_incidents = len([i for i in self._incidents.values() if i.severity == EventSeverity.CRITICAL])
        
        # Calculate event metrics by time period
        now = datetime.utcnow()
        last_hour_events = len([e for e in self._events if e.timestamp >= now - timedelta(hours=1)])
        last_day_events = len([e for e in self._events if e.timestamp >= now - timedelta(days=1)])
        ...
        return {
            ...
            'active_alert_rules': len([r for r in self._alert_rules.values() if r.enabled]),
            'event_categories': {
    ...
    def generate_security_report(self, time_range: timedelta = timedelta(days=7)) -> Dict[str, Any]:
        ...
        period_incidents = [i for i in self._incidents.values() if i.created_at >= cutoff_time]
        ...
        return {
            ...
            'summary': {
                ...
                'critical_events': len([e for e in period_events if e.severity == EventSeverity.CRITICAL]),
                'resolved_incidents': len([i for i in period_incidents if i.status == IncidentStatus.RESOLVED])
            },
