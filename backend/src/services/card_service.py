...
from src.models.user import User  # Assuming User model is available
...
class CardService:
    ...
    def _generate_card_number(self, card_type: CardType) -> str:
        ...
        # In a real system, this would be handled by a card issuer processor (e.g., Marqeta, Galileo)
        # For this refactoring, we'll generate a valid-looking number.
        # Prefix based on card type/network (e.g., 4 for Visa, 5 for Mastercard)
    ...
    def _validate_card_input(self, data: Dict[str, Any]):
    ...
    def issue_card(
        ...
        try:
            ...
            # Role-based access check: User must own the account or be an admin
            ...
            if not is_valid_luhn(card_number):
                # Should not happen with _generate_card_number, but as a safeguard
                raise CardServiceError(
            ...
            card = Card(
                ...
                # Note: ATM withdrawals and international are typically disabled by default for security
            ...
            # Set secure card data (tokenization and hashing)
            card.set_card_number(card_number)
            card.set_cvv(cvv)  # This will hash the CVV
    ...
    def activate_card(
        ...
        # For physical cards, require activation code (mock validation)
        if card.is_physical_card:
            ...
            # In a real system, this would verify against a secure, one-time code store
        ...
        card.set_pin(pin)  # This hashes the PIN
    ...
    def update_pin(
        ...
        if not is_valid:
            db.session.commit()  # Commit failed attempt count/lock status
            audit_logger.log_security_event(
    ...
    # Add other methods like freeze_card, unfreeze_card, cancel_card, update_limits, update_controls
    # ... (omitted for brevity, but would be moved from card.py route handlers)
    ...
    def update_spending_limits(
        ...
        # Logic for updating daily_limit, monthly_limit, per_transaction_limit
        # (Copied and adapted from card.py route logic)
