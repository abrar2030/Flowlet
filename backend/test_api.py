...
def test_kyc_service():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/kyc/user/create", 
...
def test_wallet_service(user_id):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/wallet/create", 
        ...
        if response.status_code == 201:
            ...
            # Test wallet balance
            response = requests.get(f"{API_BASE}/wallet/{wallet['wallet_id']}/balance", timeout=5)
...
def test_payment_service(wallet_id):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/payment/deposit", 
                               json=deposit_data, timeout=5)
...
def test_card_service(wallet_id):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/card/issue", 
...
def test_ai_service():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/ai/chatbot/query", 
...
def test_security_service():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/security/api-keys/create", 
                               json=key_data, timeout=5)
        ...
        if response.status_code == 201:
            ...
            print(f"✗ Security service test failed: {response.status_code} - {response.text}")
            return None
...
def test_ledger_service():
    ...
    try:
        # Get trial balance
        response = requests.get(f"{API_BASE}/ledger/trial-balance?currency=USD", timeout=5)
        
        if response.status_code == 200:
            result = response.json()
            print(f"✓ Trial balance generated with {len(result['trial_balance'])} accounts")
            return True
        else:
            print(f"✗ Ledger service test failed: {response.status_code} - {response.text}")
            return False
