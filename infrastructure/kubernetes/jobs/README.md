                ...
                - name: db-backup
                  ...
                  command:
                    [
                      "/bin/sh",
                      "-c",
                      "pg_dump -h your-db-host -U your-db-user your-db-name > /tmp/backup.sql && aws s3 cp /tmp/backup.sql s3://your-s3-bucket/backup-$(date +%Y%m%d%H%M%S).sql",
                    ]
                  env:
                    - name: PGPASSWORD
                      valueFrom:
                ...
                - name: log-cleanup
                  ...
                  command:
                    [
                      "/bin/sh",
                      "-c",
                      "echo 'Cleaning up old logs...' && find /var/log/app -type f -mtime +30 -delete",
                    ]
                  securityContext:
                    runAsNonRoot: true
                    readOnlyRootFilesystem: true
                ...
                - name: cert-check
                  ...
                  command:
                    [
                      "/bin/sh",
                      "-c",
                      "cert-manager check-certificates --namespace default",
                    ]
                  securityContext:
                    runAsNonRoot: true
                    readOnlyRootFilesystem: true
