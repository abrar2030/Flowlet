...
interface PCIRequirement {
  ...
  title: string;
  description: string;
  category: 'network' | 'data' | 'vulnerability' | 'access' | 'monitoring' | 'policy';
...
interface ComplianceReport {
  id: string;
  type: 'self_assessment' | 'external_audit' | 'penetration_test' | 'vulnerability_scan';
  generatedAt: string;
  period: string;
  overallScore: number;
  findings: number;
...
interface PCIDSSComplianceProps {
  ...
  onRequirementUpdate?: (requirementId: string, status: string, evidence?: string[]) => Promise<void>;
  onControlTest?: (controlId: string) => Promise<void>;
  onVulnerabilityUpdate?: (vulnerabilityId: string, status: string, notes?: string) => Promise<void>;
...
export function PCIDSSCompliance({
  ...
  const complianceMetrics = React.useMemo(() => {
    ...
    return {
      ...
      implementedControls,
      totalControls,
      controlImplementationRate: totalControls > 0 ? (implementedControls / totalControls) * 100 : 0
    };
  ...
  const filteredRequirements = React.useMemo(() => {
    return requirements.filter(req => {
      const categoryMatch = state.filterCategory === 'all' || req.category === state.filterCategory;
      const statusMatch = state.filterStatus === 'all' || req.status === state.filterStatus;
  ...
  const handleRequirementUpdate = useCallback(async (
    ...
    try {
      if (onRequirementUpdate) {
        ...
        setState(prev => ({ ...prev, success: 'Requirement updated successfully' }));
      }
    } catch (error) {
      setState(prev => ({ ...prev, error: 'Failed to update requirement' }));
    } finally {
  ...
  // Handle control test
  const handleControlTest = useCallback(async (controlId: string) => {
    setState(prev => ({ ...prev, isTesting: true, error: null }));

    try {
      if (onControlTest) {
        await onControlTest(controlId);
        setState(prev => ({ ...prev, success: 'Control test initiated successfully' }));
      }
    } catch (error) {
  ...
  const handleVulnerabilityUpdate = useCallback(async (
    ...
    try {
      if (onVulnerabilityUpdate) {
        ...
        setState(prev => ({ ...prev, success: 'Vulnerability updated successfully' }));
      }
    } catch (error) {
      setState(prev => ({ ...prev, error: 'Failed to update vulnerability' }));
    } finally {
      setState(prev => ({ ...prev, isUpdating: false }));
    }
  }, [onVulnerabilityUpdate]);
  ...
  const handleGenerateReport = useCallback(async (type: string) => {
    ...
    try {
      if (onGenerateReport) {
        ...
        setState(prev => ({ ...prev, success: 'Report generation initiated' }));
      }
    } catch (error) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'non_compliant': return 'bg-red-100 text-red-600';
      ...
      {complianceMetrics.criticalVulns > 0 && (
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Critical Alert:</strong> {complianceMetrics.criticalVulns} critical vulnerabilities require immediate attention.
          </AlertDescription>
        </Alert>
      )}
      ...
      {complianceMetrics.complianceRate < 80 && (
        <Alert className="border-amber-200 bg-amber-50">
          ...
          <AlertDescription className="text-amber-800">
            <strong>Compliance Warning:</strong> Current compliance rate is below 80%. Immediate action required.
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">Compliant Requirements</p>
                    <p className="text-2xl font-bold text-green-600">{complianceMetrics.compliant}</p>
                    <p className="text-xs text-gray-500">of {complianceMetrics.total} total</p>
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-600">{complianceMetrics.criticalVulns}</p>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">Security Controls</p>
                    <p className="text-2xl font-bold text-blue-600">{complianceMetrics.implementedControls}</p>
                    <p className="text-xs text-gray-500">of {complianceMetrics.totalControls} implemented</p>
                  </div>
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {['network', 'data', 'vulnerability', 'access', 'monitoring', 'policy'].map(category => {
                    ...
                    <div key={category} className="p-4 border rounded-lg">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>{categoryCompliant}/{categoryReqs.length}</span>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Button
                  onClick={() => setState(prev => ({ ...prev, activeTab: 'vulnerabilities' }))}
                  variant="outline"
                  className="h-20 flex flex-col items-center justify-center"
                >
                ...
                <Button
                  onClick={() => setState(prev => ({ ...prev, activeTab: 'controls' }))}
                  variant="outline"
                  className="h-20 flex flex-col items-center justify-center"
                >
                ...
                <Button
                  onClick={() => setState(prev => ({ ...prev, activeTab: 'reports' }))}
                  variant="outline"
                  className="h-20 flex flex-col items-center justify-center"
                >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-2">
                            <h4 className="font-medium">{requirement.number}: {requirement.title}</h4>
                      ...
                      <div className="flex space-x-2">
                        <Button
                          onClick={() => setState(prev => ({ ...prev, selectedRequirement: requirement }))}
                          size="sm"
                          variant="outline"
                        >
                        ...
                        {requirement.status !== 'compliant' && (
                          <Button
                            onClick={() => handleRequirementUpdate(requirement.id, 'in_progress')}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <Badge className={control.implemented ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}>
                        ...
                        <div className="space-y-2">
                          ...
                          {control.testResults && (
                            <div className="mt-2 p-2 bg-gray-50 rounded text-sm">
                              <strong>Test Results:</strong> {control.testResults}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-xs text-gray-500 mb-2">
                          ...
                          <span>Due: {new Date(vulnerability.dueDate).toLocaleDateString()}</span>
                        </div>
                      ...
                      <div className="flex space-x-2">
                        <Button
                          onClick={() => setState(prev => ({ ...prev, selectedVulnerability: vulnerability }))}
                          size="sm"
                          variant="outline"
                        >
                          <Eye className="w-3 h-3 mr-1" />
                        ...
                        {vulnerability.status === 'open' && (
                          <Button
                            onClick={() => handleVulnerabilityUpdate(vulnerability.id, 'in_progress')}
                            disabled={state.isUpdating}
                            size="sm"
                          >
                            Start Remediation
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                          ...
                          <p>Overall Score: <span className="font-medium">{report.overallScore}%</span></p>
                          <p>Findings: {report.findings} | Recommendations: {report.recommendations}</p>
                        </div>
                      </div>
                      <div className="flex space-x-2">
                        <Button
