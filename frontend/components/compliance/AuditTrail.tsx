...
interface AuditEvent {
  id: string;
  timestamp: string;
  eventType: 'authentication' | 'authorization' | 'data_access' | 'data_modification' | 'system_access' | 'configuration_change' | 'security_event' | 'compliance_event';
  action: string;
  resource: string;
  userId: string;
  userName: string;
...
interface AuditTrailProps {
  ...
  onEventExport?: (filters: AuditFilter, format: 'csv' | 'json' | 'pdf') => Promise<Blob>;
...
export function AuditTrail({
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(event => {
      // Date range filter
      if (state.filters.dateRange.start && new Date(event.timestamp) < state.filters.dateRange.start) {
        return false;
      }
      if (state.filters.dateRange.end && new Date(event.timestamp) > state.filters.dateRange.end) {
        return false;
      }

      // Event type filter
      if (state.filters.eventTypes.length > 0 && !state.filters.eventTypes.includes(event.eventType)) {
        return false;
      }

      // Outcome filter
      if (state.filters.outcomes.length > 0 && !state.filters.outcomes.includes(event.outcome)) {
        return false;
      }

      // Risk level filter
      if (state.filters.riskLevels.length > 0 && !state.filters.riskLevels.includes(event.riskLevel)) {
        return false;
      }

      // User filter
      if (state.filters.users.length > 0 && !state.filters.users.includes(event.userId)) {
        return false;
      }

      // Resource filter
      if (state.filters.resources.length > 0 && !state.filters.resources.some(r => event.resource.includes(r))) {
        return false;
      }
      ...
      if (state.filters.searchTerm) {
        ...
        const searchableText = `${event.action} ${event.resource} ${event.userName} ${event.details}`.toLowerCase();
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      if (onEventSearch) {
        ...
        setState(prev => ({ ...prev, success: `Found ${results.length} events` }));
      }
    } catch (error) {
  ...
  }, [onReportGenerate, state.filters, state.newReportName, state.newReportDescription]);
  ...
  const handleEventDetails = useCallback(async (eventId: string) => {
    try {
      if (onEventDetails) {
        ...
        setState(prev => ({ ...prev, selectedEvent: event, showEventDetails: true }));
      } else {
      ...
      setState(prev => ({ ...prev, error: 'Failed to load event details' }));
    }
  ...
  const getEventTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'system_access': return <Monitor className="w-4 h-4" />;
      case 'configuration_change': return <Settings className="w-4 h-4" />;
      case 'security_event': return <AlertTriangle className="w-4 h-4" />;
      case 'compliance_event': return <FileText className="w-4 h-4" />;
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-gray-600">
            ...
            Retention period: {retentionPolicyDays} days.
          </p>
        </CardContent>
      </Card>
      ...
      {auditMetrics.criticalEvents > 0 && (
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>Critical Alert:</strong> {auditMetrics.criticalEvents} critical security events detected.
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="space-y-4">
            <div className="flex space-x-4">
              ...
              <Button onClick={handleSearch} disabled={state.isSearching}>
                <Search className={`w-4 h-4 mr-2 ${state.isSearching ? 'animate-spin' : ''}`} />
                Search
              </Button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <DatePicker
                    ...
                    onChange={(date) => setState(prev => ({
                      ...prev,
                      filters: { ...prev.filters, dateRange: { ...prev.filters.dateRange, start: date } }
                    }))}
                  />
                </div>
                <div>
                  ...
                  <DatePicker
                    ...
                    onChange={(date) => setState(prev => ({
                      ...prev,
                      filters: { ...prev.filters, dateRange: { ...prev.filters.dateRange, end: date } }
                    }))}
                  />
                </div>
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setState(prev => ({
                      ...prev,
                      filters: { ...prev.filters, eventTypes: value === 'all' ? [] : [value] }
                    }))}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="compliance_event">Compliance</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setState(prev => ({
                      ...prev,
                      filters: { ...prev.filters, outcomes: value === 'all' ? [] : [value] }
                    }))}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setState(prev => ({
                      ...prev,
                      filters: { ...prev.filters, riskLevels: value === 'all' ? [] : [value] }
                    }))}
                  >
                    <SelectTrigger>
        ...
        <TabsContent value="events">
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-medium">Audit Events ({filteredEvents.length})</h3>
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-1">
                            <h4 className="font-medium truncate">{event.action}</h4>
                            ...
                            {event.complianceRelevant && (
                              <Badge className="bg-purple-100 text-purple-600">Compliance</Badge>
                            )}
                          </div>
                          <div className="flex items-center space-x-4 text-sm text-gray-600 mb-1">
                            ...
                            {event.deviceInfo && (
                              <span className="flex items-center">
                                ...
                                <span className="ml-1">{event.deviceInfo.type}</span>
                              </span>
                            )}
                          </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold">{auditMetrics.totalEvents}</p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-red-600">{auditMetrics.criticalEvents}</p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-orange-600">{auditMetrics.failureRate.toFixed(1)}%</p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-purple-600">{auditMetrics.complianceEvents}</p>
                    </div>
        ...
        <TabsContent value="reports">
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button
                onClick={() => setState(prev => ({ ...prev, showReportConfig: true }))}
                size="sm"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl font-bold">{retentionPolicyDays}</span>
                  ...
                  <div>
                    ...
                    <Badge className={realTimeEnabled ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'}>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-1">
                      {['Authentication', 'Authorization', 'Data Access', 'System Access', 'Security Events', 'Compliance'].map(category => (
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="capitalize">{state.selectedEvent.eventType.replace('_', ' ')}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono">{state.selectedEvent.sourceIp}</span>
                    </div>
                    {state.selectedEvent.location && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Location:</span>
                        <span>{state.selectedEvent.location.city}, {state.selectedEvent.location.country}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setState(prev => ({ ...prev, newReportName: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setState(prev => ({ ...prev, newReportDescription: e.target.value }))}
