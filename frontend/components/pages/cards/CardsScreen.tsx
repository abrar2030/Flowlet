...
const CardsScreen: React.FC<CardsScreenProps> = ({
  ...
  const handleCardAction = (action: 'view' | 'block' | 'unblock', cardId: string) => {
    switch (action) {
      case 'view':
        onViewCard?.(cardId);
        break;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card) => (
          <Card key={card.id} className="overflow-hidden">
            {/* Card Visual */}
            <div className={`bg-gradient-to-br ${getCardBrandColor(card.cardBrand)} p-6 text-white relative`}>
              {card.isVirtual && (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xl font-mono tracking-wider">
                    {showCardNumbers[card.id] ? '1234 5678 9012 3456' : card.cardNumber}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
