...
const NODE_TYPES = {
  trigger: {
    category: 'Triggers',
    items: [
      { type: 'payment_received', label: 'Payment Received', icon: DollarSign, color: 'bg-green-500' },
      { type: 'transaction_created', label: 'Transaction Created', icon: CreditCard, color: 'bg-blue-500' },
      { type: 'user_registered', label: 'User Registered', icon: Users, color: 'bg-purple-500' },
      { type: 'schedule', label: 'Schedule', icon: Clock, color: 'bg-orange-500' },
      { type: 'webhook', label: 'Webhook', icon: Globe, color: 'bg-indigo-500' },
    ]
  },
  action: {
    category: 'Actions',
    items: [
      { type: 'send_payment', label: 'Send Payment', icon: DollarSign, color: 'bg-green-600' },
      { type: 'create_card', label: 'Create Card', icon: CreditCard, color: 'bg-blue-600' },
      { type: 'send_notification', label: 'Send Notification', icon: MessageSquare, color: 'bg-yellow-600' },
      { type: 'send_email', label: 'Send Email', icon: Mail, color: 'bg-red-600' },
      { type: 'update_database', label: 'Update Database', icon: Database, color: 'bg-gray-600' },
      { type: 'generate_report', label: 'Generate Report', icon: FileText, color: 'bg-teal-600' },
    ]
  },
  condition: {
    category: 'Logic',
    items: [
      { type: 'if_condition', label: 'If Condition', icon: GitBranch, color: 'bg-amber-500' },
      { type: 'filter', label: 'Filter', icon: Filter, color: 'bg-cyan-500' },
      { type: 'calculator', label: 'Calculator', icon: Calculator, color: 'bg-pink-500' },
  ...
  security: {
    ...
    items: [
      { type: 'fraud_check', label: 'Fraud Check', icon: Shield, color: 'bg-red-500' },
      { type: 'compliance_check', label: 'Compliance Check', icon: CheckCircle, color: 'bg-emerald-500' },
      { type: 'risk_assessment', label: 'Risk Assessment', icon: AlertTriangle, color: 'bg-orange-600' },
      { type: 'kyc_verification', label: 'KYC Verification', icon: Users, color: 'bg-violet-500' },
    ]
  },
  analytics: {
    category: 'Analytics',
    items: [
      { type: 'track_event', label: 'Track Event', icon: BarChart3, color: 'bg-blue-700' },
      { type: 'analytics_report', label: 'Analytics Report', icon: BarChart3, color: 'bg-green-700' },
    ]
  }
};
...
const WorkflowDesigner: React.FC<WorkflowDesignerProps> = ({ workflowId }) => {
  ...
  const deleteNode = useCallback((nodeId: string) => {
    ...
    const updatedWorkflow = {
      ...
      connections: currentWorkflow.connections.filter(c => c.source !== nodeId && c.target !== nodeId),
      updatedAt: new Date()
    };
            ...
            {currentWorkflow && (
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onChange={(e) => {
                    const updatedWorkflow = { ...currentWorkflow, name: e.target.value };
                    setCurrentWorkflow(updatedWorkflow);
                    setWorkflows(prev => prev.map(w => w.id === currentWorkflow.id ? updatedWorkflow : w));
                  }}
                  className="w-48"
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className={`p-2 rounded ${nodeInfo.color} text-white`}>
                        <Icon className="h-4 w-4" />
                      </div>
                      <div className="flex-1">
            ...
            {currentWorkflow && currentWorkflow.nodes.length === 0 && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm">Drag nodes from the palette to begin creating your financial workflow</p>
                </div>
              </div>
            )}
          </div>
        ...
        return (
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select value={config.currency || 'USD'} onValueChange={(value) => onUpdate({ ...config, currency: value })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
        ...
        return (
          <div className="space-y-3">
            ...
            <div>
              ...
              <Select value={config.priority || 'normal'} onValueChange={(value) => onUpdate({ ...config, priority: value })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Slider
                ...
                onValueChange={([value]) => onUpdate({ ...config, riskThreshold: value })}
                max={100}
                step={1}
                className="mt-2"
              />
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => onUpdate({ ...config, blockSuspicious: checked })}
              />
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                onChange={(e) => onUpdate({ ...config, duration: e.target.value })}
                placeholder="5"
              />
            </div>
            <div>
              <Label>Unit</Label>
              <Select value={config.unit || 'minutes'} onValueChange={(value) => onUpdate({ ...config, unit: value })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
